...
prek run --all-files         # clang-format, cmake-format, shellcheck, ruff, basics
```

Config: `.pre-commit-config.yaml` + `.clang-format` (first-party `main/` `src/` `include/` `tests/` only; `third_party/` excluded).
