...
void gcta::mbat_calcu_lambda(vector<int>& snp_indx,
                             MatrixXf& rval,
                             VectorXd& eigenval,
                             int& snp_count,
...
void gcta::mbat_ACATO(double& mbat_svd_pvalue, double& fastbat_pvalue, double& P_mBATcombo) {
    ...
    for (int k = 0; k < 2; k++) {
        if (combinedPvalue[k] == 1)
            combinedPvalue[k] = 1 - 1e-16;
        if (combinedPvalue[k] < 1e-16)
            isSmall[k] = 1;
    }

    if (isSmall.sum() == 0) {
        for (int k = 0; k < 2; k++)
            cctStat += tan((0.5 - combinedPvalue[k]) * M_PI);
    } else if (isSmall.sum() == 1) {
        for (int k = 0; k < 2; k++) {
            if (isSmall[k] == 0)
                cctStat = (1 / combinedPvalue[1 - k]) / M_PI + tan((0.5 - combinedPvalue[k]) * M_PI);
            else
                cctStat = (1 / combinedPvalue[k]) / M_PI + tan((0.5 - combinedPvalue[1 - k]) * M_PI);
        }
    } else {
        for (int k = 0; k < 2; k++)
            cctStat += (1 / combinedPvalue[k]) / M_PI;
    }
    cctStat = cctStat / 2;

    if (cctStat > 1e+15)
        P_mBATcombo = (1 / cctStat) / M_PI;
    else
        P_mBATcombo = 1 - (((1.0L / M_PI) * atan((cctStat - 0L) / 1.0L) + 0.5L));
}
...
void gcta::svdDecomposition(MatrixXf& X, double& prop, int& eigenvalueNum, VectorXd& eigenvalueUsed, MatrixXd& U_prop) {
    ...
    revIdx = revIdx - 1;
    if (eigenVal(revIdx) < 0)
        cout << "Error, all eigenvector are negative" << endl;
...
void gcta::mbat_gene(string mbat_sAssoc_file,
    ...
    cout << " StatFunc::pchisq(1e100, 1):" << to_string(StatFunc::pchisq(1e100, 1)) << " " << StatFunc::pchisq(1e100, 1)
         << endl;
    ...
    /////////////////////////////////////////
    LOGGER << "Mapping the physical positions of genes to SNP data (gene boundaries: " << mbat_wind / 1000
...
#pragma omp parallel for private(iter, chr_iter)
    ...
    LOGGER << "For computing fastBAT test statistics, SNPs are pruned with LD rsq cutoff of = "
           << sbat_ld_cutoff * sbat_ld_cutoff << " (set by --fastBAT-ld-cutoff 0.9 as default)." << endl;
    LOGGER << "For computing mBAT test statistics, top principal components are selected to explain at least "
           << mbat_svd_gamma * 100 << "% of variance in LD (set by --mBAT-svd-gamma 0.9 as default)." << endl;
    ...
    SnpNameMap::iterator iter1, iter2;
    ...
    ofile << "Gene\tChr\tStart\tEnd\tNo.SNPs\tSNP_start\tSNP_end\tTopSNP\tTopSNP_Pvalue\tNo.Eigenvalues\tChisq_mBAT\tP_"
             "mBATcombo";
    if (mbat_print_all_p) {
    ...
    for (j = 0; j < mapped; j++) {
        ...
        bool skip = false;
        if (iter1 == _snp_name_map.end() || iter2 == _snp_name_map.end() || iter1->second >= iter2->second)
            skip = true;
        //
        ...
        if (!skip && snp_num_in_gene[j] > 10000) {
            LOGGER << "The following genes have > 10000 SNPs which may take at least a few minutes for each test. You "
                      "may consider removing these genes from the input list if it takes too much time."
                   << endl;
    ...
    for (j = 0; j < mapped; j++) {
        ...
        snp_num_in_gene_mBAT[j] = snp_num_in_gene[j];
        // if(!skip && snp_num_in_gene[j] > 10000){
        //    LOGGER<<"Warning: Too many SNPs in the gene region [" << gene_name[gene_ori_idx] << "]. Maximum limit is
        //    10000. This gene is ignored in the analysis."<<endl;
        // skip = true;
        // }
        if (skip) {
            P_mBATcombo[j] = 2.0;
        ...
        if (snp_num_in_gene[j] == 1) {
            ...
            for (int k = 0; k < snp_indx.size(); k++) {
                if (min_snp_pval[j] > _pval[include_in_gene[snp_indx[k]]]) {
                    ...
                    min_snp_name[j] =
                        _snp_name[snp_indx[k]]; // keep minimum value - regardless of whether SNP removed by LD pruning
                }
        ...
        // Step 4. output
        /////////////////////////////////////////
        if (P_mBATcombo[j] > 1.5)
            continue;
        gene_analyzed++;
        ofile << gene_name[gene_ori_idx] << "\t" << gene_chr[gene_ori_idx] << "\t" << gene_bp1[gene_ori_idx] << "\t"
              << gene_bp2[gene_ori_idx] << "\t";
        ofile << snp_num_in_gene_mBAT[j] << "\t" << gene2snp_1[gene_ori_idx] << "\t" << gene2snp_2[gene_ori_idx]
              << "\t";
        ofile << min_snp_name[j] << "\t" << min_snp_pval[j] << "\t" << eigenvalueNum_mBAT[j] << "\t" << Chisq_mBAT[j]
              << "\t" << P_mBATcombo[j];

        if (mbat_print_all_p) {
            ofile << "\t" << P_mbat_svd_prop[j] << "\t" << chisq_o[j] << "\t" << fastbat_gene_pvalue[j];
        }
        ofile << endl;
        // LOGGER << "mBAT-combo analysis for " << gene_ori_idx+1 << "/"<< gene_num << "-th gene [" <<
        // gene_name[gene_ori_idx] << "] has been completed." << endl;
    }
    ofile.close();
    rogoodsnp.close();
}

void gcta::mbat(string mbat_sAssoc_file,
                string snpset_file,
                double mbat_svd_gamma,
                double sbat_ld_cutoff,
                bool mbat_write_snpset,
    ...
    for (i = 0; i < set_num; i++) {
        ...
        if (!skip && snp_num_in_set[i] > 10000) {
            LOGGER << "The following sets have > 10000 SNPs which may take at least a few minutes for each test. You "
                      "may consider removing these genes from the input list if it takes too much time."
                   << endl;
            // for (int k = 0; k < snp_indx.size(); k++) {
    ...
    for (i = 0; i < set_num; i++) {
        ...
        //     LOGGER<<"Warning: Too many SNPs in the set ["<<set_name[i]<<"]. Maximum limit is 20000. This gene is
        //     ignored in the analysis."<<endl; skip = true;
        // }
        if (skip) {
            P_mBATcombo[i] = 2.0;
            snp_num_in_set[i] = 0;
        ...
        // Step 4. output
        /////////////////////////////////////////
        if (P_mBATcombo[i] > 1.5)
            continue;
        gene_analyzed++;
        ofile << set_name[i] << "\t" << snp_num_in_set_mBAT[i] << "\t";
        ofile << min_snp_name[i] << "\t" << min_snp_pval[i] << "\t" << eigenvalueNum_mBAT[i] << "\t" << Chisq_mBAT[i]
              << "\t" << P_mBATcombo[i];

        if (mbat_print_all_p) {
            ofile << "\t" << P_mbat_svd_prop[i] << "\t" << chisq_o[i] << "\t" << fastbat_set_pvalue[i];
        }
        ofile << endl;
        // LOGGER << "mBAT-combo analysis for " << gene_ori_idx+1 << "/"<< gene_num << "-th gene [" <<
        // gene_name[gene_ori_idx] << "] has been completed." << endl;
