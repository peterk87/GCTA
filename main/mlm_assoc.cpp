...
void gcta::mlma(string grm_file,
                bool m_grm_flag,
    ...
    if (!qcovar_flag && !covar_flag)
        no_adj_covar = false;
    if (m_grm_flag)
        grm_flag = false;
    bool subtract_grm_flag = (!subtract_grm_file.empty());
    if (subtract_grm_flag && m_grm_flag)
        LOGGER.e(0, "the --mlma-subtract-grm option cannot be used in combination with the --mgrm option.");
    ...
    if (subtract_grm_flag) {
        ...
        if (grm_flag) {
            ...
            for (i = 0; i < _keep.size(); i++)
                grm_id.push_back(_fid[_keep[i]] + ":" + _pid[_keep[i]]);
        }
    }
...
#pragma omp parallel for private(k)
        for (j = 0; j < _n; j++) {
            for (k = 0; k <= j; k++) {
                if (kp[j] >= kp[k])
                    (_A[0])(k, j) = (_A[0])(j, k) =
                        ((_A[0])(j, k) * A_N_buf(j, k) - _grm(kp[j], kp[k]) * _grm_N(kp[j], kp[k])) /
                        (A_N_buf(j, k) - _grm_N(kp[j], kp[k]));
                else
                    (_A[0])(k, j) = (_A[0])(j, k) =
                        ((_A[0])(j, k) * A_N_buf(j, k) - _grm(kp[k], kp[j]) * _grm_N(kp[k], kp[j])) /
                        (A_N_buf(j, k) - _grm_N(kp[k], kp[j]));
            }
        }
        _grm.resize(0, 0);
        _grm_N.resize(0, 0);
    } else {
        for (i = 0; i < grm_files.size() + 1; i++)
            _r_indx.push_back(i);
        _A.resize(_r_indx.size());
        if (grm_flag) {
            StrFunc::match(uni_id, grm_id, kp);
...
#pragma omp parallel for private(j)
            ...
            LOGGER << "There are " << grm_files.size() << " GRM file names specified in the file [" + grm_file + "]."
                   << endl;
            for (i = 0; i < grm_files.size(); i++) {
...
#pragma omp parallel for private(k)
                for (j = 0; j < _n; j++) {
                    for (k = 0; k <= j; k++) {
                        if (kp[j] >= kp[k])
                            (_A[i])(k, j) = (_A[i])(j, k) = _grm(kp[j], kp[k]);
                        else
                            (_A[i])(k, j) = (_A[i])(j, k) = _grm(kp[k], kp[j]);
                    }
                }
            }
        } else {
...
#pragma omp parallel for private(j)
            for (i = 0; i < _n; i++) {
                for (j = 0; j <= i; j++)
                    (_A[0])(j, i) = (_A[0])(i, j) = _grm_mkl[kp[i] * _n + kp[j]];
            }
            delete[] _grm_mkl;
        }
    }
    ...
    // run REML algorithm
    LOGGER << "\nPerforming MLM association analyses" << (subtract_grm_flag ? "" : " (including the candidate SNP)")
           << " ..." << endl;
    ...
    if (!no_adj_covar)
        y_buf = _y.array() - (_X * _b).array(); // adjust phenotype for covariates
    for (i = 0; i < n; i++)
        y[i] = y_buf[i];
    ...
    if (no_adj_covar)
        mlma_calcu_stat_covar(y, _geno_mkl, n, m, beta, se, pval);
    else
        mlma_calcu_stat(y, _geno_mkl, n, m, beta, se, pval);
    delete[] y;
    delete[] _geno_mkl;

    string filename = _out + ".mlma";
    LOGGER << "\nSaving the results of the mixed linear model association analyses of " << m
           << " SNPs to [" + filename + "] ..." << endl;
    ...
    for (i = 0; i < m; i++) {
        ...
        if (pval[i] > 1.5)
            ofile << "NA\tNA\tNA\tNA" << endl;
        else
            ofile << 0.5 * _mu[j] << "\t" << beta[i] << "\t" << se[i] << "\t" << pval[i] << endl;
    }
    ofile.close();
}

void gcta::mlma_calcu_stat(float* y,
                           float* geno_mkl,
                           unsigned long n,
                           unsigned long m,
                           eigenVector& beta,
...
#pragma omp parallel for private(j)
    ...
    for (i = 0; i < m; i++, block_col++) {
        ...
        if (i == new_start) {
            ...
            for (k = i, l = 0; l < block_size; k++, l++)
                indx[l] = k;
            make_XMat_subset(X_block, indx, false);
        }

        for (j = 0; j < n; j++)
            X[j] = X_block(j, block_col);
...
void gcta::mlma_calcu_stat_covar(float* y,
                                 float* geno_mkl,
                                 unsigned long n,
                                 unsigned long m,
                                 eigenVector& beta,
...
#pragma omp parallel for private(j)
    ...
    for (i = 0; i < m; i++, block_col++) {
        ...
        for (j = 0; j < n; j++)
            X[j * col_num + _X_c] = X_block(j, block_col);
        cblas_sgemm(
            CblasRowMajor, CblasNoTrans, CblasNoTrans, n, col_num, n, 1.0, Vi, n, X, col_num, 0.0, Vi_X, col_num);
        cblas_sgemm(CblasRowMajor,
                    ...
                    Xt_Vi_X,
                    col_num);
        if (!comput_inverse_logdet_LU_mkl_array(col_num, Xt_Vi_X, d_buf))
            LOGGER.e(0, "Xt_Vi_X is not invertible.");
...
void gcta::mlma_loco(string phen_file,
                     string qcovar_file,
                     string covar_file,
                     int mphen,
                     int MaxIter,
    ...
    if (vi_buf.size() < 2)
        LOGGER.e(0,
                 "There is only one chromosome. The MLM leave-on-chromosome-out (LOCO) analysis requires at least two "
                 "chromosomes.");
    for (i = 0; i < vi_buf.size(); i++) {
        if (vi_buf[i] <= _autosome_num)
            chrs.push_back(vi_buf[i]);
    }
    vector<int> include_o(_include);
    SnpNameMap snp_name_map_o(_snp_name_map);
    ...
    if (_mu.empty())
        calcu_mu();
    LOGGER << "\nCalculating the genetic relationship matrix for each of the " << chrs.size() << " chromosomes ... "
           << endl;
...
#pragma omp parallel for private(j)
        ...
        if (no_adj_covar)
            mlma_calcu_stat_covar(y, (geno_chrs[c1]), n, _include.size(), beta[c1], se[c1], pval[c1]);
        else
    ...
    LOGGER << "\nSaving the results of the mixed linear model association analyses of " << _include.size()
           << " SNPs to [" + filename + "] ..." << endl;
    ofstream ofile(filename.c_str());
    if (!ofile)
        LOGGER.e(0, "cannot open the file [" + filename + "] to write.");
    ...
    for (c1 = 0; c1 < chrs.size(); c1++) {
        for (i = 0; i < icld_chrs[c1].size(); i++) {
            ...
            if (pval[c1][i] > 1.5)
                ...
                ofile << 0.5 * _mu[j] << "\t" << beta[c1][i] << "\t" << se[c1][i] << "\t" << pval[c1][i] << endl;
        }
    }
    ofile.close();
}
