...
void gcta::gbat_read_snpAssoc(string snpAssoc_file,
...
void gcta::gbat(string sAssoc_file, string gAnno_file, int wind, int simu_num) {
    ...
    for (i = 0; i < snp_pval.size(); i++)
        snp_chisq[i] = StatFunc::qchisq(snp_pval[i], 1);
    ...
    // map genes to SNPs
    LOGGER << "Mapping the physical positions of genes to SNP data (gene bounaries: " << wind / 1000
