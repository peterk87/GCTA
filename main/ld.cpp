...
void gcta::read_LD_target_SNPs(string snplistfile) {
    ...
    for (i = 0; i < _snp_num; i++)
        snp_map_buf.insert(pair<string, int>(_snp_name[i], i));
    for (i = _ld_target_snp.size() - 1; i >= 0; i--) {
        iter = snp_map_buf.find(_ld_target_snp[i]);
        if (iter != snp_map_buf.end())
            _snp_name_map.insert(*iter);
        else
            _ld_target_snp.erase(_ld_target_snp.begin() + i);
    }
    _include.clear();
    for (auto snp_it = _snp_name_map.begin(); snp_it != _snp_name_map.end(); ++snp_it)
        _include.push_back(snp_it->second);
    stable_sort(_include.begin(), _include.end());
    if (_ld_target_snp.size() == 0)
        LOGGER.e(0, "no target SNP is retained to estimate the LD structure.");
    else
        LOGGER << prev_target_snp_num << " target SNPs are read from [" + snplistfile + "], " << _ld_target_snp.size()
               << " of which exist in the data." << endl;
}
...
void gcta::LD_Blocks(int stp, double wind_size, double alpha, bool IncldQ, bool save_ram) {
    ...
    if (!SavFile)
        LOGGER.e(0, "cannot open the file [" + SavFileName + "] to save result!");
    SavFile << "target_SNP\tfreq\tL_region\tR_region\tL_snp\tR_snp\tnSNPs\tmean_rsq\tmedian_rsq\tmax_rsq\tmax_rsq_snp"
            << endl;
    for (i = 0; i < SNP_SmplNum; i++)
        SavFile << _snp_name[_include[smpl[i]]] << "\t" << 0.5 * _mu[_include[smpl[i]]] << "\t" << dL[i] << "\t"
                << dR[i] << "\t" << L_SNP[i] << "\t" << R_SNP[i] << "\t" << K[i] << "\t" << r2[i] << "\t" << md_r2[i]
                << "\t" << max_r2[i] << "\t" << max_r2_snp[i] << endl;
    SavFile.close();
    ...
    if (!SavFile)
        LOGGER.e(0, "cannot open the file [" + SavFileName + "] to save result.");
    for (i = 0; i < SNP_SmplNum; i++) {
        for (j = 0; j < snp_ls[i].size(); j++)
            SavFile << snp_ls[i][j] << " ";
        SavFile << endl;
    }
    SavFile.close();
    LOGGER << "Results have been saved in [" + _out + ".rsq.ld]" + ", [" + _out + ".r.ld]" + " and [" + _out +
                  ".snp.ld].\n"
           << endl;
}

void gcta::EstLD(vector<int>& smpl,
                 double wind_size,
    ...
    for (i = 0; i < smpl.size(); i++)
        smpl_snp_map.insert(pair<int, int>(_include[smpl[i]], i));

    LOGGER << "Parameters used to search for SNPs in LD with the given SNPs: window size=" << (int)(wind_size * 0.001)
           << "Kb, significant level=" << alpha << endl;
    ...
    for (i = 0; i < smpl.size(); i++) {
        ...
        while (1) {
            if (R == _include.size() - 1)
                break;
            if (_chr[_include[R]] != _chr[_include[R + 1]])
                break;
            if (_bp[_include[R + 1]] - _bp[_include[smpl[i]]] > wind_size)
                break;
            R++;
            if (smpl_snp_map.find(_include[R]) != smpl_snp_map.end())
                continue;
            makex(R, x);
            reg(y, x, rst);
            if (rst[2] < alpha) {
        ...
        while (1) {
            if (L == 0)
                break;
            if (_chr[_include[L]] != _chr[_include[L - 1]])
                break;
            if (_bp[_include[smpl[i]]] - _bp[_include[L - 1]] > wind_size)
                break;
            L--;
            if (smpl_snp_map.find(_include[L]) != smpl_snp_map.end())
                continue;
            makex(L, x);
            reg(y, x, rst);
            if (rst[2] < alpha) {
...
eigenMatrix gcta::reg(vector<double>& y, vector<double>& x, vector<double>& rst, bool table) {
    int N = x.size();
    if (N != y.size() || N < 1)
        LOGGER.e(0, "The lengths of x and y do not match.");
...
void gcta::rm_cor_snp(int m, int start, float* rsq, double rsq_cutoff, vector<int>& rm_snp_ID1) {
    ...
    for (i = 0; i < rm_uni_ID.size(); i++) {
        i_buf = count(rm_snp_ID1.begin(), rm_snp_ID1.end(), rm_uni_ID[i]) +
                count(rm_snp_ID2.begin(), rm_snp_ID2.end(), rm_uni_ID[i]);
...
void gcta::calcu_mean_rsq(int wind_size, double rsq_cutoff, bool dominance_flag) {
    ...
    LOGGER << "\nCalculating LD score for SNPs (block size of " << wind_size / 1000 << "Kb with an overlap of "
           << wind_size / 2000 << "Kb between blocks); LD rsq threshold = " << rsq_cutoff << ") ... " << endl;
    if (dominance_flag)
        LOGGER << "(SNP genotypes are coded for dominance effects)" << endl;
    if (_ldscore_adj)
        LOGGER << "LD rsq will be adjusted for chance correlation, i.e. rsq_adj = rsq - (1 - rsq) / (n -2)." << endl;
    ...
    if (brk_pnt2.size() > 1)
        calcu_ld_blk(brk_pnt2, brk_pnt3, mean_rsq, snp_num, max_rsq, true, rsq_cutoff, dominance_flag);

    string mrsq_file = "";
    if (dominance_flag)
        mrsq_file = _out + ".d.score.ld";
    else
...
void gcta::calcu_ssx_sqrt_i(eigenVector& ssx_sqrt_i) {
    ...
    for (i = 0; i < m; i++) {
        ...
        if (ssx_sqrt_i[i] < 1.0e-30)
            ssx_sqrt_i[i] = 0.0;
        else
            ssx_sqrt_i[i] = 1.0 / sqrt(ssx_sqrt_i[i]);
    }
}

void gcta::get_ld_blk_pnt(
    ...
    for (i = 1, j = 0; i < m; i++) {
        if (i == (m - 1)) {
            if (chr_start ||
                ((_bp[_include[i]] - _bp[_include[brk_pnt1[j]]] > 0.5 * wind_bp) && (i - brk_pnt1[j] > 0.5 * wind_snp)))
            ...
            if (chr_start || ((_bp[_include[i - 1]] - _bp[_include[brk_pnt1[j]]] > 0.5 * wind_bp) &&
...
void gcta::calcu_ld_blk(vector<int>& brk_pnt,
                        vector<int>& brk_pnt3,
    ...
    for (i = 0; i < brk_pnt.size() - 1; i++) {
        if (_chr[_include[brk_pnt[i]]] != _chr[_include[brk_pnt[i + 1]]])
            continue;
        ...
        MatrixXf X_sub;
        if (dominance_flag)
            make_XMat_d_subset(X_sub, snp_indx, true);
        else
...
#pragma omp parallel for private(k)
            for (j = 0; j < size; j++) {
                ...
                for (k = 0; k < size; k++) {
                    ...
                    if (rsq_sub(j, k) >= rsq_cutoff) {
                        if (_ldscore_adj)
                            mean_rsq_sub[j] += rsq_sub(j, k) - (1.0 - rsq_sub(j, k)) / (n - 2.0);
                        else
                            mean_rsq_sub[j] += rsq_sub(j, k);
                        rsq_size[j] += 1.0;
                    }
                    if (rsq_sub(j, k) > max_rsq_sub[j])
                        max_rsq_sub[j] = rsq_sub(j, k);
                }
                if (rsq_size[j] > 0.0)
                    mean_rsq_sub[j] /= rsq_size[j];
            }
        }
        ...
        for (j = 0, k = brk_pnt[i]; j < size; j++, k++) {
            if (second) {
                if (rsq_size[j] > 0.0) {
                    mean_rsq[k] =
                        (mean_rsq[k] * snp_num[k] + mean_rsq_sub[j] * rsq_size[j]) / (snp_num[k] + rsq_size[j]);
                    snp_num[k] = (snp_num[k] + rsq_size[j]);
                    if (max_rsq[k] < max_rsq_sub[j])
...
void gcta::calcu_ld_blk_split(int size,
                              int size_limit,
                              MatrixXf& X_sub,
    ...
    for (i = 0; i < brk_pnt_sub.size() - 1; i++) {
        ...
        eigenVector rsq_size_sub(size_sub), mean_rsq_sub_sub(size_sub),
            max_rsq_sub_sub = eigenVector::Constant(size_sub, -1.0);
        ...
        for (j = 0; j < size_sub; j++) {
            ...
            for (k = 0; k < size; k++) {
                ...
                if (rsq_sub_sub(j, k) >= rsq_cutoff) {
                    if (_ldscore_adj)
                        mean_rsq_sub_sub[j] += rsq_sub_sub(j, k) - (1.0 - rsq_sub_sub(j, k)) / (n - 2.0);
                    else
                        mean_rsq_sub_sub[j] += rsq_sub_sub(j, k);
                    rsq_size_sub[j] += 1.0;
                }
                if (rsq_sub_sub(j, k) > max_rsq_sub_sub[j])
                    max_rsq_sub_sub[j] = rsq_sub_sub(j, k);
            }

            if (rsq_size_sub[j] > 0.0)
                mean_rsq_sub_sub[j] /= rsq_size_sub[j];
        }
...
void gcta::calcu_mean_rsq_multiSet(string snpset_filenames_file,
                                   int wind_size,
                                   double rsq_cutoff,
                                   bool dominance_flag) {
    check_autosome();
    ...
    if (!in_snpset_filenames)
        LOGGER.e(0, "cannot open the file [" + snpset_filenames_file + "] to read.");
    string str_buf;
    vector<string> snpset_filenaems, vs_buf;
    while (getline(in_snpset_filenames, str_buf)) {
        if (!str_buf.empty()) {
            if (StrFunc::split_string(str_buf, vs_buf) == 1)
                snpset_filenaems.push_back(vs_buf[0]);
        }
    }
    ...
    if (set_num < 1)
        LOGGER.e(0, "no filename found in [" + snpset_filenames_file + "].");

    vector<vector<string>> snplist(set_num);
    for (i = 0; i < set_num; i++) {
        ifstream i_snplist(snpset_filenaems[i].c_str());
        if (!i_snplist)
            LOGGER.e(0, "cannot open the file [" + snpset_filenaems[i] + "] to read.");
    ...
    for (i = 0; i < m; i++)
        snp_map.insert(pair<string, int>(_snp_name[_include[i]], i));
    ...
    LOGGER << "\nCalculating multi-component LD score for SNPs (block size of " << wind_size / 1000
           << "Kb with an overlap of " << wind_size / 2000 << "Kb between blocks); LD rsq threshold = " << rsq_cutoff
           << ") ... " << endl;
    if (dominance_flag)
        LOGGER << "(SNP genotypes are coded for dominance effects)" << endl;
    if (_ldscore_adj)
    ...
    if (brk_pnt2.size() > 1)
        calcu_ld_blk_multiSet(
            brk_pnt2, brk_pnt3, set_flag, mean_rsq, snp_num, max_rsq, true, rsq_cutoff, dominance_flag);

    string mrsq_file = "";
    double ldscore = 0.0;
    if (dominance_flag)
        mrsq_file = _out + ".d.multi_score.ld";
    else
        mrsq_file = _out + ".multi_score.ld";
    ofstream o_mrsq(mrsq_file.data());
    o_mrsq << "SNP chr bp MAF";
    for (j = 0; j < set_num; j++)
        o_mrsq << " mean_rsq_" << j + 1 << " snp_num_" << j + 1 << " max_rsq" << j + 1 << " ldscore" << j + 1;
    o_mrsq << endl;
    for (i = 0; i < m; i++) {
        o_mrsq << _snp_name[_include[i]] << " " << _chr[_include[i]] << " " << _bp[_include[i]] << " ";
        double MAF = 0.5 * _mu[_include[i]];
...
void gcta::calcu_ld_blk_multiSet(vector<int>& brk_pnt,
                                 vector<int>& brk_pnt3,
        ...
        for (s = 0; s < set_num; s++) {
            ...
            for (j = 0, k = brk_pnt[i]; j < size; j++, k++) {
                if (set_flag[s][k] == true) {
                    ...
                    set_flag_sub[j] = false;
            }

            int size_of_this_set = used_in_this_set.size();
            if (size_of_this_set < 1)
                continue;

            MatrixXf X_sub2(n, size_of_this_set);
            for (j = 0; j < size_of_this_set; j++)
                X_sub2.col(j) = X_sub.col(used_in_this_set[j]);
            ...
            if (size > size_limit)
                calcu_ld_blk_split_multiSet(size,
                                            size_limit,
                                            X_sub,
                                            X_sub2,
                                            used_in_this_set,
...
#pragma omp parallel for private(k, l)
            ...
            for (j = 0, k = brk_pnt[i]; j < size; j++, k++) {
                if (second) {
                    if (rsq_size[j] > 0.0) {
                        (mean_rsq[s])[k] = ((mean_rsq[s])[k] * (snp_num[s])[k] + mean_rsq_sub[j] * rsq_size[j]) /
                                           ((snp_num[s])[k] + rsq_size[j]);
                        (snp_num[s])[k] = ((snp_num[s])[k] + rsq_size[j]);
                        if ((max_rsq[s])[k] < max_rsq_sub[j])
...
void gcta::calcu_ld_blk_split_multiSet(int size,
                                       int size_limit,
                                       MatrixXf& X_sub,
                                       MatrixXf& X_sub2,
                                       vector<int>& used_in_this_set,
...
void gcta::ld_seg(string i_ld_file, int seg_size, int wind_size, double rsq_cutoff, bool dominance_flag) {
    ...
    if (!i_ld_file.empty()) {
        ...
        if (!ild)
            LOGGER.e(0, "cannot open the file [" + i_ld_file + "] to read.");
        ...
        while (ild) {
            ild >> str_buf;
            if (ild.eof() || str_buf.size() < 1)
                break;
            _snp_name[i] = str_buf;
            if (!(ild >> _chr[i]))
                LOGGER.e(0, "in the file [" + i_ld_file + "].");
            if (!(ild >> _bp[i]))
                LOGGER.e(0, "in the file [" + i_ld_file + "].");
            if (!(ild >> _mu[i]))
                LOGGER.e(0, "in the file [" + i_ld_file + "].");
        ...
        m = _include.size();
        check_autosome();
        LOGGER << "\nCalculating LD score between SNPs (block size of " << wind_size / 1000 << "Kb with an overlap of "
               << wind_size / 2000 << "Kb between blocks); LD rsq threshold = " << rsq_cutoff << ") ... " << endl;
        if (dominance_flag)
            LOGGER << "(SNP genotypes are coded for dominance effects)" << endl;
        get_ld_blk_pnt(brk_pnt1, brk_pnt2, brk_pnt3, wind_size);
        eigenVector mrsq_buf = eigenVector::Zero(m), snp_num_buf = eigenVector::Zero(m),
                    max_rsq_buf = eigenVector::Zero(m);
        calcu_ld_blk(brk_pnt1, brk_pnt3, mrsq_buf, snp_num_buf, max_rsq_buf, false, rsq_cutoff, dominance_flag);
        if (brk_pnt2.size() > 1)
            calcu_ld_blk(brk_pnt2, brk_pnt3, mrsq_buf, snp_num_buf, max_rsq_buf, true, rsq_cutoff, dominance_flag);
        mrsq.resize(m);
        snp_num.resize(m);
        max_rsq.resize(m);
        ldscore.resize(m);
    ...
    LOGGER << "Calculating regional mean LD score (region width =  " << seg_size / 1000 << "Kb with an overlap of "
           << seg_size / 2000 << "Kb between regions) ... " << endl;
    ...
    for (i = 0; i < brk_pnt1.size() - 1; i++) {
        ...
        if (size < 3)
            continue;
        double ld_score = 0.0;
        for (j = brk_pnt1[i]; j <= brk_pnt1[i + 1]; j++)
            ld_score += ldscore[j]; // mrsq[j] * snp_num[j] + 1.0;
    ...
    for (i = 0; i < brk_pnt2.size() - 1; i++) {
        ...
        if (size < 3)
            continue;
        double ld_score = 0.0;
        for (j = brk_pnt2[i]; j <= brk_pnt2[i + 1]; j++)
            ld_score += ldscore[j]; // mrsq[j] * snp_num[j] + 1.0;
        ld_score /= (double)size;
        for (j = brk_pnt2[i]; j <= brk_pnt2[i + 1]; j++)
            lds[j] = 0.5 * (ld_score + lds[j]);
    }

    string lds_file;
    if (dominance_flag)
        lds_file = _out + ".d.score.ld";
    else
        lds_file = _out + ".score.ld";
    ...
    for (i = 0; i < m; i++) {
        o_lds << _snp_name[_include[i]] << " " << _chr[_include[i]] << " " << _bp[_include[i]] << " "
              << 0.5 * _mu[_include[i]];
...
void gcta::calcu_max_ld_rsq(int wind_size, double rsq_cutoff, bool dominance_flag) {
    ...
    LOGGER << "Calculating maximum LD rsq between SNPs (block size of " << wind_size / 1000 << "Kb with an overlap of "
           << wind_size / 2000 << "Kb between blocks; LD rsq threshold = " << rsq_cutoff << ") ... " << endl;
    LOGGER << "(Maximum number of SNPs allowed in a block = " << max_size << " due to computational limitation)"
           << endl;
    if (dominance_flag)
        LOGGER << "(SNP genotypes are coded for dominance effects)" << endl;
    ...
    eigenVector multi_rsq = eigenVector::Constant(m, -1.0), max_rsq = eigenVector::Constant(m, -999),
                multi_rsq_adj = eigenVector::Constant(m, -999);
    vector<int> max_pos(m);
    calcu_max_ld_rsq_blk(multi_rsq, multi_rsq_adj, max_rsq, max_pos, brk_pnt1, rsq_cutoff, dominance_flag);
    if (brk_pnt2.size() > 1)
        calcu_max_ld_rsq_blk(multi_rsq, multi_rsq_adj, max_rsq, max_pos, brk_pnt2, rsq_cutoff, dominance_flag);

    string max_rsq_file = "";
    if (dominance_flag)
        max_rsq_file = _out + ".d.max_rsq.ld";
    else
        max_rsq_file = _out + ".max_rsq.ld";
    ofstream o_max_rsq(max_rsq_file.data());
    o_max_rsq << "SNP freq max_rsq max_snp multi_rsq multi_rsq_adj" << endl;
    for (i = 0; i < m; i++) {
        if (max_rsq[i] > 0.0 || multi_rsq[i] > -998)
            o_max_rsq << _snp_name[_include[i]] << " " << 0.5 * _mu[_include[i]] << " " << max_rsq[i] << " "
                      << _snp_name[_include[max_pos[i]]] << " " << multi_rsq[i] << " " << multi_rsq_adj[i] << endl;
        else
            o_max_rsq << _snp_name[_include[i]] << " " << 0.5 * _mu[_include[i]] << " NA NA NA NA" << endl;
    }
    o_max_rsq << endl;
    LOGGER << "Maximum LD rsq for " << m << " SNPs have been saved in the file [" + max_rsq_file + "]." << endl;
}

void gcta::get_ld_blk_pnt_max_limit(
    vector<int>& brk_pnt1, vector<int>& brk_pnt2, vector<int>& brk_pnt3, int wind_bp, int wind_snp) {
    unsigned long i = 0, j = 0, k = 0, m = _include.size();
...
void gcta::calcu_max_ld_rsq_blk(eigenVector& multi_rsq,
...
#pragma omp parallel for private(k)
        ...
        for (j = 0; j < size; j++)
            Q_diag(j) = R_i.col(j).dot(rsq_sub.row(j).transpose());
        VectorXf multi_rsq_buf(size);
        for (j = 0; j < size; j++) {
            if (fabs(Q_diag[j] - 1.0) < 0.01)
                multi_rsq_buf[j] = 1.0 - 1.0 / R_i(j, j);
            else
                multi_rsq_buf[j] = 1.0;
            if (multi_rsq_buf[j] > 1.0)
                multi_rsq_buf[j] = 1.0;
        }
        VectorXf multi_rsq_buf_adj =
            multi_rsq_buf.array() - (1.0 - multi_rsq_buf.array()) * (eff_m / ((double)n - eff_m - 1.0));
        ...
        for (j = 0; j < size; j++) {
            ...
            if (multi_rsq_buf[j] < max_rsq_buf[j])
                multi_rsq_buf[j] = max_rsq_buf[j];
        }
