...
void gcta::paa(string aa_file) {
    ...
    if (!i_aa)
        LOGGER.e(0, "cannot open the file [" + aa_file + "] to read.");
    string cbuf = ".";
    string str_buf;
    ...
    double x = 0.0;
    vector<double> hom_aa_rare(_keep.size()), hom_aa_comm(_keep.size()), hom_da_rare(_keep.size()),
        hom_da_comm(_keep.size()), het_aa_rare(_keep.size()), het_aa_comm(_keep.size()), nomiss(_keep.size());
    ...
    if (!o_paa)
        LOGGER.e(0, "cannot open the file [" + paa_file + "] to write.");
    o_paa << "FID\tIID\tNOMISS\tHOM_AA_RARE\tHOM_AA_COMM\tHOM_DA_RARE\tHOM_DA_COMM\tHET_AA_RARE\tHET_AA_COMM" << endl;
    for (i = 0; i < _keep.size(); i++)
        o_paa << _fid[i] << "\t" << _pid[i] << "\t" << nomiss[i] << "\t" << hom_aa_rare[i] << "\t" << hom_aa_comm[i]
              << "\t" << hom_da_rare[i] << "\t" << hom_da_comm[i] << "\t" << het_aa_rare[i] << "\t" << het_aa_comm[i]
              << endl;
    o_paa.close();
...
void gcta::ibc(bool ibc_all) {
    ...
    vector<double> h(_include.size()), h_i(_include.size()), w(_include.size()), p(_include.size()),
        p_q(_include.size()), q_p(_include.size()); // variance of each SNP, 2pq
#pragma omp parallel for
    for (j = 0; j < _include.size(); j++) {
        p[j] = 0.5 * _mu[_include[j]];
        h[j] = 2.0 * p[j] * (1.0 - p[j]);
        p_q[j] = p[j] / (1.0 - p[j]);
        if (fabs(p_q[j]) < 1.0e-50)
            q_p[j] = 0.0;
        else
            q_p[j] = 1.0 / p_q[j];
        w[j] = h[j] / (1.0 - h[j]);
...
#pragma omp parallel for private(k)
    for (i = 0; i < _keep.size(); i++) {
        ...
        for (k = 0; k < _include.size(); k++) {
            if (!_snp_1[_include[k]][_keep[i]] || _snp_2[_include[k]][_keep[i]]) {
                x = _snp_1[_include[k]][_keep[i]] + _snp_2[_include[k]][_keep[i]];
                if (_allele2[_include[k]] == _ref_A[_include[k]])
                    x = 2.0 - x;
    ...
    if (!o_ibc)
        LOGGER.e(0, "cannot open the file [" + ibc_file + "] to write.");
    if (ibc_all) {
        o_ibc << "FID\tIID\tNOMISS\tP_RARE_HOM\tP_COMM_HOM\tFhat1\tFhat1_w\tFhat2\tFhat2_"
                 "w\tFhat3\tFhat4\tFhat5\tFhat6\tFhat7"
              << endl;
        for (i = 0; i < _keep.size(); i++)
            o_ibc << _fid[_keep[i]] << "\t" << _pid[_keep[i]] << "\t" << nomiss[i] << "\t" << rare_hom[i] << "\t"
                  << comm_hom[i] << "\t" << Fhat1[i] - 1.0 << "\t" << Fhat1_w[i] - 1.0 << "\t" << Fhat2[i] << "\t"
                  << Fhat2_w[i] << "\t" << Fhat3[i] << "\t" << Fhat4[i] - 1.0 << "\t" << Fhat5[i] << "\t" << Fhat6[i]
                  << "\t" << Fhat7[i] << endl;
    } else {
        o_ibc << "FID\tIID\tNOMISS\tFhat1\tFhat2\tFhat3" << endl;
        for (i = 0; i < _keep.size(); i++)
            o_ibc << _fid[_keep[i]] << "\t" << _pid[_keep[i]] << "\t" << nomiss[i] << "\t" << Fhat1[i] - 1.0 << "\t"
                  << Fhat2[i] << "\t" << Fhat3[i] << endl;
    }
    o_ibc.close();
...
void gcta::Fst(string filename) {
    ...
    coeff_mat(subpopu,
              S,
              "too many subpopulations specified in the file [" + filename + "].",
              "at least two sub-populations are required.");
    int r = subpopu_name.size();
    eigenVector x(_keep.size()), tmp, n_sub(r), xt_S, p_sub(r),
        h_sub(r); // Fst(_include.size()),  f_all(_include.size()), chisq(_include.size()),
                  // pval=eigenVector::Constant(_include.size(), 1.0);
    if (_mu.empty())
        calcu_mu();
    ...
    if (!ofile)
        LOGGER.e(0, "cannot open the file [" + outfile + "] to write.");
    ofile << "Chr\tSNP\tbp\trefA\t";
    for (i = 0; i < r; i++)
        ofile << "freq_" << subpopu_name[i] << "(n=" << n_sub[i] << ")\t";
    ofile << "Fst" << endl;
    double p_bar = 0.0, s_sq = 0.0, h_bar = 0.0, a = 0.0, b = 0.0, c = 0.0, Fst = 0.0, d_buf = 0.0;
    for (i = 0; i < _include.size(); i++) {
        ofile << _chr[_include[i]] << "\t" << _snp_name[_include[i]] << "\t" << _bp[_include[i]] << "\t"
              << _ref_A[_include[i]] << "\t";
...
void gcta::read_subpopu(string filename, vector<string>& subpopu, vector<string>& subpopu_name) {
    ...
    if (!ifstream_subpopu)
        LOGGER.e(0, "cannot open the file [" + filename + "] to read.");
    ...
    for (i = 0; i < ID.size(); i++) {
        ...
        if (iter != uni_id_map.end())
            subpopu[iter->second] = subpopu_buf[i][0];
    }
...
void gcta::std_XMat_subpopu(
    string subpopu_file, MatrixXf& X, eigenVector& sd_SNP, bool grm_xchr_flag, bool miss_with_mu, bool divid_by_std) {
    vector<string> subpopu, subpopu_name;
    ...
    coeff_mat(subpopu,
              S,
              "too many subpopulations specified in the file [" + subpopu_file + "].",
              "at least two sub-populations are required.");
            ...
            if (divid_by_std) {
                if (_dosage_flag) {
                    ...
                    for (i = 0; i < n; i++) {
                        if (X(i, j) < 1e5 && S(i, popu) > 0.0)
                            (sd_SNP_sub[popu])(j) +=
                                (X(i, j) - (mu_SNP_sub[popu])(j)) * (X(i, j) - (mu_SNP_sub[popu])(j));
                    }
                    (sd_SNP_sub[popu])(j) /= (m_sub - 1.0);
                } else
                    (sd_SNP_sub[popu])(j) = (mu_SNP_sub[popu])(j) * (1.0 - 0.5 * (mu_SNP_sub[popu])(j));
                if ((sd_SNP_sub[popu])(j) < 1.0e-50)
                    (sd_SNP_sub[popu])(j) = 0.0;
                else
                    (sd_SNP_sub[popu])(j) = 1.0 / sqrt((sd_SNP_sub[popu])(j));
            }
        }
    }
