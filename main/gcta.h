...
// SNP ID → panel index. Hash map: BIM init / COJO keep-filter are O(n) hot paths.
using SnpNameMap = std::unordered_map<std::string, int>;
...
class gcta {
  public:
    ...
    void make_grm(bool grm_d_flag,
                  bool grm_xchr_flag,
                  bool inbred,
                  bool output_bin,
                  int grm_mtd,
                  bool mlmassoc,
                  bool diag_f3_flag = false,
                  string subpopu_file = "");
    // void make_grm_pca(bool grm_d_flag, bool grm_xchr_flag, bool inbred, bool output_bin, int grm_mtd, double
    // wind_size, bool mlmassoc);
    void save_grm(string grm_file,
                  string keep_indi_file,
                  string remove_indi_file,
                  string sex_file,
                  double grm_cutoff,
                  double adj_grm_fac,
                  int dosage_compen,
    ...
    // bigK + smallK method
    void
    grm_bK(string grm_file, string keep_indi_file, string remove_indi_file, double threshold, bool grm_out_bin_flag);

    void enable_grm_bin_flag();
    void fit_reml(string grm_file,
                  string phen_file,
                  string qcovar_file,
                  string covar_file,
                  string qGE_file,
                  ...
                  bool reml_bending = false,
                  bool reml_diag_one = false,
                  string weight_file = "");
    // void HE_reg(string grm_file, string phen_file, string keep_indi_file, string remove_indi_file, int mphen); // old
    // HE regression method
    void HE_reg(string grm_file,
                bool m_grm_flag,
                string phen_file,
                string keep_indi_file,
                string remove_indi_file,
                int mphen); // allow multiple regression
    void HE_reg_bivar(string grm_file,
                      bool m_grm_flag,
                      string phen_file,
                      string keep_indi_file,
                      string remove_indi_file,
                      int mphen,
                      int mphen2); // estimate genetic covariance between two traits
    void blup_snp_geno();
    ...
    void fit_bivar_reml(string grm_file,
                        string phen_file,
                        string qcovar_file,
                        string covar_file,
                        string keep_indi_file,
                        ...
                        bool no_constrain,
                        bool ignore_Ce,
                        vector<double>& fixed_rg_val,
                        bool bivar_no_constrain);
    ...
    void run_massoc_slct(string metafile,
                         int wind_size,
                         double p_cutoff,
                         double collinear,
                         int64_t top_SNPs,
                         ...
                         bool actual_geno,
                         int mld_slct_alg);
    void run_massoc_cond(
        string metafile, string snplistfile, int wind_size, double collinear, bool GC, double GC_val, bool actual_geno);
    ...
    // mkl
    void make_grm_mkl(bool grm_d_flag,
                      bool grm_xchr_flag,
                      bool inbred,
                      bool output_bin,
                      int grm_mtd,
    ...
    // void make_grm_wt_mkl(string i_ld_file, int wind_m, double wt_ld_cut, bool grm_xchr_flag, bool inbred, bool
    // output_bin, int grm_mtd=0, int wt_mtd=0, bool mlmassoc=false, bool impData_flag=false, int ttl_snp_num=-1);

    // mlma
    void mlma(string grm_file,
              bool m_grm_flag,
              ...
              bool inbred,
              bool no_adj_covar);
    void mlma_loco(string phen_file,
                   string qcovar_file,
                   string covar_file,
                   int mphen,
                   int MaxIter,
    ...
    void sbat_gene(string sAssoc_file,
                   string gAnno_file,
                   int sbat_wind,
                   ...
                   double GC_val);
    void
    sbat(string sAssoc_file, string snpset_file, double sbat_ld_cutoff, bool sbat_write_snpset, bool GC, double GC_val);
    void
    sbat_seg(string sAssoc_file, int seg_size, double sbat_ld_cutoff, bool sbat_write_snpset, bool GC, double GC_val);

    // gene based association
    //////////////////////////////
    void mbat_gene(string mbat_sAssoc_file,
                   string mbat_gAnno_file,
                   int mbat_wind,
                   double mbat_svd_gamma,
                   double sbat_ld_cutoff,
                   bool mbat_write_snpset,
                   bool GC,
                   double GC_val,
                   bool mbat_print_all_p);
    ...
    void mbat_calcu_lambda(vector<int>& snp_indx,
                           MatrixXf& rval,
                           VectorXd& eigenval,
                           int& snp_count,
                           double sbat_ld_cutoff,
                           vector<int>& sub_indx);
    void mbat(string mbat_sAssoc_file,
              string snpset_file,
              double mbat_svd_gamma,
              double sbat_ld_cutoff,
              bool mbat_write_snpset,
    ...
    // GSMR
    void
    read_gsmrfile(string expo_file_list, string outcome_file_list, double gwas_thresh, int nsnp_gsmr, int gsmr_so_alg);
    void gsmr(int gsmr_alg_flag,
              string ref_ld_dirt,
              string w_ld_dirt,
              double freq_thresh,
              double gwas_thresh,
              double clump_wind_size,
              double clump_r2_thresh,
              double std_heidi_thresh,
              double global_heidi_thresh,
              double ld_fdr_thresh,
              int nsnp_gsmr,
              bool o_snp_instru_flag,
              int gsmr_so_alg,
              int gsmr_beta_version);
    vector<vector<double>> forward_gsmr(stringstream& ss,
                                        ...
                                        double ld_fdr_thresh,
                                        int nsnp_gsmr,
                                        stringstream& ss_pleio);
    vector<vector<double>> reverse_gsmr(stringstream& ss,
                                        map<string, int>& snp_instru_map,
                                        double gwas_thresh,
                                        double clump_wind_size,
                                        double clump_r2_thresh,
                                        ...
                                        int nsnp_gsmr,
                                        stringstream& ss_pleio);
    eigenMatrix rho_sample_overlap(vector<vector<bool>> snp_val_flag,
                                   eigenMatrix snp_b,
                                   eigenMatrix snp_se,
    ...
    eigenMatrix sample_overlap_ldsc(vector<vector<bool>> snp_val_flag,
                                    eigenMatrix snp_b,
                                    eigenMatrix snp_se,
                                    eigenMatrix snp_n,
                                    int nexpo,
                                    ...
                                    string w_ld_dirt,
                                    vector<string> trait_name);
    eigenMatrix sample_overlap_rb(vector<vector<bool>> snp_val_flag,
                                  eigenMatrix snp_b,
                                  eigenMatrix snp_se,
                                  eigenMatrix snp_pval,
                                  eigenMatrix snp_n,
    ...
    // mtCOJO
    void mtcojo(string mtcojo_bxy_file,
                string ref_ld_dirt,
                string w_ld_dirt,
                double freq_thresh,
                double gwas_thresh,
                ...
                int nsnp_gsmr,
                int gsmr_beta_version);
    bool mtcojo_ldsc(vector<vector<bool>> snp_val_flag,
                     eigenMatrix snp_b,
                     eigenMatrix snp_se,
                     eigenMatrix snp_n,
                     int ntrait,
    ...
    double read_single_metafile_txt(string metafile,
                                    ...
                                    eigenVector& snp_n,
                                    vector<bool>& snpflag);
    double read_single_metafile_gz(string metafile,
                                   map<string, int> id_map,
                                   vector<string>& snp_a1,
                                   vector<string>& snp_a2,
                                   eigenVector& snp_freq,
  ...
  private:
    ...
    void update_include(vector<int> chr_buf,
                        ...
                        int file_indx);
    void update_keep(vector<string> fid_buf,
    ...
    void update_id_map_kp(const vector<string>& id_list, MapT& id_map, vector<int>& keep);
    template <typename MapT>
    void update_id_map_rm(const vector<string>& id_list, MapT& id_map, vector<int>& keep);
    ...
    void std_XMat_subpopu(string subpopu_file,
                          MatrixXf& X,
                          eigenVector& sd_SNP,
                          bool grm_xchr_flag,
                          bool miss_with_mu,
    ...
    void read_grm(string grm_file,
                  vector<string>& grm_id,
                  bool out_id_log = true,
                  bool read_id_only = false,
                  bool dont_read_N = false);
    void read_grm_gz(string grm_file, vector<string>& grm_id, bool out_id_log = true, bool read_id_only = false);
    void read_grm_bin(string grm_file,
                      vector<string>& grm_id,
                      bool out_id_log = true,
                      bool read_id_only = false,
                      bool dont_read_N = false);
    ...
    void manipulate_grm(string grm_file,
                        string keep_indi_file,
                        string remove_indi_file,
                        string sex_file,
                        double grm_cutoff,
    ...
    // reml
    void
    read_phen(string phen_file, vector<string>& phen_ID, vector<vector<string>>& phen_buf, int mphen, int mphen2 = 0);
    ...
    void construct_X(int n,
                     map<string, int>& uni_id_map,
                     bool qcovar_flag,
                     int qcovar_num,
                     vector<string>& qcovar_ID,
                     vector<vector<string>>& qcovar,
                     bool covar_flag,
                     int covar_num,
                     vector<string>& covar_ID,
                     vector<vector<string>>& covar,
    ...
    void reml(bool pred_rand_eff,
              ...
              bool no_lrt,
              bool mlmassoc = false);
    double reml_iteration(eigenMatrix& Vi_X,
    ...
    void ai_reml(
        eigenMatrix& P, eigenMatrix& Hi, eigenVector& Py, eigenVector& prev_varcmp, eigenVector& varcmp, double dlogL);
    ...
    void calcu_hsq(int i,
                   ...
                   eigenMatrix& Hi);
    void
    calcu_sum_hsq(double Vp, double VarVp, double& sum_hsq, double& var_sum_hsq, eigenVector& varcmp, eigenMatrix& Hi);
    ...
    // GWAS simulation
    void kosambi();
    int read_QTL_file(string qtl_file,
    ...
    void GenerCases(string bfile,
                    string qtl_file,
                    int case_num,
                    int control_num,
                    double hsq,
                    double K,
                    bool curr_popu = false,
                    double gnrt = 100);

    // LD
    void EstLD(vector<int>& smpl,
               double wind_size,
               vector<vector<string>>& snp,
               vector<vector<double>>& r,
               vector<double>& r2,
               vector<double>& md_r2,
               vector<double>& max_r2,
               vector<string>& max_r2_snp,
               vector<double>& dL,
               vector<double>& dR,
               vector<int>& K,
               vector<string>& L_SNP,
    ...
    void
    get_ld_blk_pnt(vector<int>& brk_pnt1, vector<int>& brk_pnt2, vector<int>& brk_pnt3, int wind_bp, int wind_snp = 0);
    void get_ld_blk_pnt_max_limit(
        vector<int>& brk_pnt1, vector<int>& brk_pnt2, vector<int>& brk_pnt3, int wind_bp, int wind_snp);
    void get_lds_brkpnt(vector<int>& brk_pnt1, vector<int>& brk_pnt2, int ldwt_seg, int wind_snp_num = 0);
    void calcu_ld_blk(vector<int>& brk_pnt,
                      vector<int>& brk_pnt3,
                      eigenVector& mean_rsq,
                      eigenVector& snp_num,
                      eigenVector& max_rsq,
                      bool second,
                      double rsq_cutoff,
                      bool dominance_flag = false);
    void calcu_ld_blk_split(int size,
                            int size_limit,
                            MatrixXf& X_sub,
                            eigenVector& ssx_sqrt_i_sub,
                            double rsq_cutoff,
                            eigenVector& rsq_size,
                            eigenVector& mean_rsq_sub,
                            eigenVector& max_rsq_sub,
                            int s1,
                            int s2,
                            bool second);
    void calcu_ssx_sqrt_i(eigenVector& ssx_sqrt_i);
    void calcu_max_ld_rsq_blk(eigenVector& multi_rsq,
                              eigenVector& multi_rsq_adj,
                              eigenVector& max_rsq,
                              vector<int>& max_pos,
                              vector<int>& brk_pnt,
                              double rsq_cutoff,
    ...
    void calcu_ld_blk_multiSet(vector<int>& brk_pnt,
                               vector<int>& brk_pnt3,
                               vector<vector<bool>>& set_flag,
                               vector<eigenVector>& mean_rsq,
                               vector<eigenVector>& snp_num,
                               vector<eigenVector>& max_rsq,
                               bool second,
                               double rsq_cutoff,
                               bool dominance_flag);
    void calcu_ld_blk_split_multiSet(int size,
                                     int size_limit,
                                     MatrixXf& X_sub,
                                     MatrixXf& X_sub2,
    ...
    void
    vec_t_mat(float* vec, int nrow, float* mat, int ncol, eigenVector& out); // 1 x n vector multiplied by m x n matrix
    void stepwise_slct(vector<int>& slct,
                       vector<int>& remain,
                       eigenVector& bC,
                       eigenVector& bC_se,
                       eigenVector& pC,
    ...
    void z_cache_ensure_physical_rows(int need_rows, int m);
    void z_cache_reset();
    void z_cache_rebuild_col_has_nz();
    ...
    void std_XMat_mkl(
        float* X, vector<double>& sd_SNP, bool grm_xchr_flag, bool miss_with_mu = false, bool divid_by_std = true);
    ...
    void calcu_ld_blk_mkl(float* X,
                          ...
                          bool second,
                          double rsq_cutoff);
    void calcu_ld_blk_split_mkl(int size,
                                int size_limit,
                                float* X_sub,
                                vector<double>& ssx_sub,
                                double rsq_cutoff,
                                vector<double>& rsq_size,
                                vector<double>& mean_rsq_sub,
                                vector<double>& max_rsq_sub,
                                int s1,
                                int s2,
                                bool second);

    // mlma
    void mlma_calcu_stat(float* y,
                         float* geno_mkl,
                         unsigned long n,
                         unsigned long m,
                         eigenVector& beta,
                         eigenVector& se,
                         eigenVector& pval);
    void mlma_calcu_stat_covar(float* y,
                               float* geno_mkl,
                               unsigned long n,
                               unsigned long m,
                               eigenVector& beta,
    ...
    void calcu_ld_blk_ldwt(eigenVector &ssx_sqrt_i, vector<int> &brk_pnt, vector<int> &brk_pnt3, eigenVector &mean_rsq,
    eigenVector &snp_num, eigenVector &max_rsq, bool second, double rsq_cutoff, bool adj); void
    calcu_ld_blk_split_ldwt(int size, int size_limit, int s_pnt, eigenVector &ssx_sqrt_i_sub, double rsq_cutoff,
    eigenVector &rsq_size, eigenVector &mean_rsq_sub, eigenVector &max_rsq_sub, int s1, int s2, bool second, bool adj);
    ...
    void calcu_ldak_blk(eigenVector &wt, eigenVector &sum_rsq, eigenVector &ssx_sqrt_i, vector<int> &brk_pnt, bool
    second, double rsq_cutoff); void calcu_ldwt(string i_ld_file, eigenVector &wt, int wind_size, double rsq_cutoff);
    void read_mrsq_mb(string i_ld_file, vector<float> &seq, vector<double> &mrsq_mb, eigenVector &wt, eigenVector
    &snp_m); void adj_wt_4_maf(eigenVector &wt); void cal_sum_rsq_mb(eigenVector &sum_rsq_mb); void col_std(MatrixXf
    &X); void assign_snp_2_mb(vector<float> &seq, vector< vector<int> > &maf_bin_pos, int mb_num); void
    make_grm_pca_blk(vector<int> & maf_bin_pos_i, int ldwt_seg, double &trace); void glpk_simplex_solver(MatrixXf &rsq,
    eigenVector &mrsq, eigenVector &wt, int maxiter); void calcu_grm_wt_mkl(string i_ld_file, float *X, vector<double>
    &sd_SNP, eigenVector &wt, int wind_size, double rsq_cutoff, int wt_mtd, int ttl_snp_num);
    */

    // gene based association test
    void sbat_read_snpAssoc(string snpAssoc_file,
                            vector<string>& snp_name,
                            vector<int>& snp_chr,
                            vector<int>& snp_bp,
                            vector<double>& snp_pval);
    void sbat_read_geneAnno(string gAnno_file,
                            vector<string>& gene_name,
                            vector<int>& gene_chr,
                            vector<int>& gene_bp1,
                            vector<int>& gene_bp2);
    void sbat_read_snpset(string snpset_file, vector<string>& set_name, vector<vector<string>>& snpset);
    void sbat_calcu_lambda(
        vector<int>& snp_indx, VectorXd& eigenval, int& snp_count, double sbat_ld_cutoff, vector<int>& sub_indx);
    void get_sbat_seg_blk(int seg_size,
    ...
    // gene based association test
    void gbat_read_snpAssoc(string snpAssoc_file,
                            vector<string>& snp_name,
                            vector<int>& snp_chr,
                            vector<int>& snp_bp,
                            vector<double>& snp_pval);
    void gbat_read_geneAnno(string gAnno_file,
                            vector<string>& gene_name,
                            vector<int>& gene_chr,
                            vector<int>& gene_bp1,
                            vector<int>& gene_bp2);
    ...
    void init_gwas_variable(vector<vector<string>>& snp_a1,
    ...
    void update_meta_snp_map(vector<string> snplist,
    ...
    vector<string> remove_bad_snps(vector<string> snp_name,
                                   vector<int> snp_remain,
                                   ...
                                   int ncovar,
                                   string outfile_name);
    vector<string> remove_freq_diff_snps(vector<string> meta_snp_name,
                                         ...
                                         double freq_thresh,
                                         string outfile_name);
    vector<string> remove_mono_snps(const SnpNameMap& snp_name_map, vector<double> ref_snpfreq, string outfile_name);
    vector<string> filter_meta_snp_pval(vector<string> snp_name,
                                        ...
                                        vector<vector<bool>> snp_flag,
                                        double pval_thresh);
    vector<double> gsmr_meta(vector<string>& snp_instru,
                             eigenVector bzx,
                             eigenVector bzx_se,
                             ...
                             string& err_msg);
    vector<string>
    clumping_meta(eigenVector snp_chival, vector<bool> snp_flag, double pval_thresh, int wind_size, double r2_thresh);
    void update_mtcojo_snp_rm(vector<string> adjsnps, map<string, int>& snp_id_map, vector<int>& remain_snp_indx);
    vector<string> read_snp_ldsc(map<string, int> ldsc_snp_name_map,
                                 vector<string> snp_name,
                                 vector<int> snp_remain,
                                 int& ttl_mk_num,
                                 string ref_ld_dirt,
    ...
    void reorder_snp_effect(vector<int> snp_remain,
                            eigenMatrix& bhat_z,
                            eigenMatrix& bhat_n,
                            eigenMatrix snp_b,
                            eigenMatrix snp_se,
                            ...
                            vector<double> w_ld_vec,
                            int ntrait);
    eigenMatrix ldsc_snp_h2(eigenMatrix bhat_z,
                            eigenMatrix bhat_n,
                            eigenVector ref_ld,
                            eigenVector w_ld,
                            vector<vector<bool>> snp_flag,
                            ...
                            vector<string> trait_name,
                            int ntrait);
    eigenMatrix ldsc_snp_rg(eigenMatrix ldsc_var_h2,
                            eigenMatrix bhat_z,
                            eigenMatrix bhat_n,
                            eigenVector ref_ld,
                            eigenVector w_ld,
    ...
    void adjust_snp_effect_for_pc(
        eigenVector& bzy_adj, eigenVector& bzx_hat, eigenVector bzy, eigenVector bxy_hat, int wind_size);
  ...
  private:
    ...
    // Logical rows are compact [0, _Z_nrows); storage may have geometric capacity.
    eigenMatrix _Z_cache;
    eigenMatrix _Z_N_cache;
    bool _Z_cache_ready = false;
    int _Z_nrows = 0;           // logical rows (== #selected / B dimension)
    vector<char> _Z_col_has_nz; // size m: column has any nonzero (skip in massoc_cond)
    ...
    // Stepwise COJO profiling (accumulated over one --cojo-slct run).
    long long _cojo_prof_massoc_cond_ns = 0;
    long long _cojo_prof_insert_ok_ns = 0;
    long long _cojo_prof_insert_reject_ns = 0;
    long long _cojo_prof_slct_stay_ns = 0;
    long long _cojo_prof_n_massoc_cond = 0;
    long long _cojo_prof_n_insert_ok = 0;
    long long _cojo_prof_n_insert_reject = 0;
    void reset_cojo_profile();
    void report_cojo_profile();
