...
void gcta::read_pc_adjust_file(string pcadjust_list_file, string pc_file) {
    ...
    if (!in_lambda)
        LOGGER.e(0, "cannot open the file [" + eigenvalue_file + "] to read.");
    while (std::getline(in_lambda, strbuf)) {
        ...
        if (line_elements.size() > 1) {
            LOGGER.w(0,
                     "Only the first element would be accepted. File [" + eigenvalue_file + "], line " +
    ...
    if (!meta_list)
        LOGGER.e(0, "cannot open the file [" + pcadjust_list_file + "] to read.");
    while (std::getline(meta_list, strbuf)) {
        ...
        if (line_elements.size() > 1) {
            LOGGER.w(0,
                     "Only the first element would be accepted. File [" + pcadjust_list_file + "], line " +
                         to_string(line_number) + ".");
        }
    ...
    if (ncovar > _eigen_value.size())
        LOGGER.e(0,
                 "there are summary data for " + to_string(ncovar) + " covariates, but " +
                     to_string(_eigen_value.size()) + " eigenvalues are provided.");
    ...
    for (i = 0; i <= ncovar; i++) {
        ...
        if (pheno_file[i].substr(pheno_file[i].length() - 3, 3) != ".gz")
            _meta_vp_trait(i) = read_single_metafile_txt(pheno_file[i],
                                                         _meta_snp_name_map,
                                                         snp_a1[i],
                                                         snp_a2[i],
                                                         snp_freq_buf,
            ...
            _meta_vp_trait(i) = read_single_metafile_gz(pheno_file[i],
                                                        _meta_snp_name_map,
                                                        snp_a1[i],
                                                        snp_a2[i],
                                                        snp_freq_buf,
        ...
        if (_meta_vp_trait(i) < 0) {
            if (i == 0)
                LOGGER.e(0, "negative phenotypic variance of the target trait.");
            else
                LOGGER.e(0, "negative phenotypic variance of the covariate #" + to_string(i + 1) + ".");
        }
    ...
    LOGGER.i(0,
             to_string(_include.size()) + " SNPs are in common between the summary data and the LD reference sample.");
}

vector<string> update_snp_freq(vector<string> meta_snp_name,
    ...
    for (i = 0; i < nsnp; i++) {
        ...
        for (j = 0; j < npheno; j++) {
            ...
            if (!std::isnan(meta_freq(meta_snp_remain[i], j))) {
                if (!freq_flag)
                    a1_freq = meta_freq(meta_snp_remain[i], j);
                freq_flag = true;
            }
    ...
    if (!afsnps.empty()) {
        ...
        LOGGER.i(0,
                 to_string(nafsnps) +
                     " SNP(s) do not have allele frequency information. These SNPs have been saved in [" + afsnpfile +
                     "].");
...
double init_pc_meta(vector<int> snp_remain,
    ...
    for (i = 0; i < m1; i++) {
        ...
        snp_n_o(snp_remain[i]) = (vp_trait - vf * snp_b(snp_remain[i]) * snp_b(snp_remain[i])) /
                                     (vf * snp_se(snp_remain[i]) * snp_se(snp_remain[i])) +
                                 1.0;
    }
...
double est_bxy_pc(vector<int> snp_remain,
                  eigenVector bzy,
                  eigenVector bzy_freq,
                  eigenVector& bzx,
                  eigenVector bzx_freq,
                  eigenVector bzx_n,
                  double n_o,
                  int ttl_mk_num,
                  double eigen_value) {
...
void restrict_snp_effect(map<string, int> meta_snp_name_map,
...
#pragma omp parallel for
    for (int i = 0; i < m; i++) {
        ...
        bzy(i) = snp_b(iter->second, 0);
        bzy_se(i) = snp_se(iter->second, 0);
    ...
    snp_freq.resize(m, 1);
    snp_b.resize(m, ncovar);
    snp_se.resize(m, ncovar);
    snp_n_o.resize(m, ncovar);
    snp_freq = snp_freq_tmp;
    snp_b = snp_b_tmp;
    snp_se = snp_se_tmp;
    snp_n_o = snp_n_tmp;
}

void init_ld_snp_index(
    int snp_start, int& snp_end, vector<int> snp_remain, vector<int> snp_chr, vector<int> snp_bp, int wind_size) {
    ...
    while (1) {
        ...
        if (snp_chr[snp_test_end] == snp_chr[snp_test_start] &&
            abs(snp_bp[snp_test_end] - snp_bp[snp_test_start]) < wind_size) {
            continue;
        } else
            break;
    }
...
double calcu_ztz_product_b(map<string, int> meta_snp_name_map,
                           vector<string> snp_name,
                           vector<int> snp_remain,
                           eigenVector snp_delta,
                           eigenVector ztz_vec,
                           double ztz_target,
                           int snp_target,
                           int snp_wind_start,
    ...
    for (i = snp_wind_start; i <= snp_wind_end; i++) {
        if (abs(snp_bp[snp_remain[snp_target]] - snp_bp[snp_remain[i]]) >= wind_size)
            continue;
        snp_test_start = i;
        break;
    }
    ...
    eigenVector ztz_tmp = ztz_vec.segment(snp_test_start, nsnp_test),
                delta = snp_delta.segment(snp_test_start, nsnp_test);
...
void gcta::adjust_snp_effect_for_pc(
    ...
    while (1) {
        int m_buf1 = snp_buf1_end - snp_buf1_start + 1, m_in = snp_end - snp_start + 1,
            m_buf2 = snp_buf2_end - snp_buf2_start + 1;
        ...
        // LOGGER<<"snp "<<snp_buf1_start << " "<<m_buf1 << " "<< snp_start<<" "<< m_in << " "<< snp_buf2_start<<"
        // "<<m_buf2<<endl;
        //  adjust SNP effect
        ...
        dbuf3.setZero(m_in);
        if (m_buf1 > 1)
            dbuf1 = (ztz_buf1 * snp_delta.segment(snp_buf1_start, m_buf1)).array() /
                    msx.segment(snp_start, m_in).array().sqrt();
        dbuf2 = (ztz_in * snp_delta.segment(snp_start, m_in)).array() / msx.segment(snp_start, m_in).array().sqrt();
        if (m_buf2 > 1)
            dbuf3 = (ztz_buf2 * snp_delta.segment(snp_buf2_start, m_buf2)).array() /
                    msx.segment(snp_start, m_in).array().sqrt();
        ...
        t3.setZero(m_in);
        if (m_buf1 > 1)
            t1 = (ztz_buf1 * snp_delta2.segment(snp_buf1_start, m_buf1)).array() /
                 msx.segment(snp_start, m_in).array().sqrt();
        t2 = (ztz_in * snp_delta2.segment(snp_start, m_in)).array() / msx.segment(snp_start, m_in).array().sqrt();
        if (m_buf2 > 1)
            t3 = (ztz_buf2 * snp_delta2.segment(snp_buf2_start, m_buf2)).array() /
                 msx.segment(snp_start, m_in).array().sqrt();
        ...
        if (end_chr_flag1) {
            ...
            snp_buf1_start = snp_start;
            snp_buf1_end = snp_end;
            snp_start = snp_buf2_start;
            snp_end = snp_buf2_end;
...
void output_snp_effect_for_pc(string output_file,
                              vector<string> meta_snp_name,
                              vector<int> meta_snp_remain,
                              const SnpNameMap& snp_name_map,
                              vector<string> snp_name,
    ...
    for (i = 0; i < meta_nsnp; i++) {
        ...
        SnpNameMap::const_iterator iter = snp_name_map.find(snpbuf);
        if (iter == snp_name_map.end())
            continue;
        int i_buf = iter->second;
        ofile << snp_name[i_buf] << "\t" << snp_a1[meta_snp_remain[i]] << "\t" << snp_a2[meta_snp_remain[i]] << "\t"
              << snp_freq(i_buf) << "\t" << snp_b(i_buf) << "\t" << snp_se(i_buf) << "\t"
              << snp_pval(meta_snp_remain[i]) << "\t" << snp_n(i_buf) << "\t" << snp_b_adj(i_buf) << "\t"
              << bzx_hat(i_buf) << endl;
    }
    ofile.close();
}
...
void gcta::pc_adjust(string pcadjust_list_file, string pc_file, double freq_thresh, int wind_size) {
    ...
    afsnps = remove_freq_diff_snps(
        _meta_snp_name, _meta_remain_snp, _snp_name_map, _mu, _meta_snp_freq, _snp_val_flag, npheno, freq_thresh, _out);
    // Update SNPs set
    if (afsnps.size() > 0) {
    ...
    afsnps = update_snp_freq(
        _meta_snp_name, _meta_remain_snp, _snp_name_map, _mu, _meta_snp_freq, _snp_val_flag, npheno, _out);
    // Update SNPs set
    if (afsnps.size() > 0) {
        update_id_map_rm(afsnps, _snp_name_map, _include);
        update_mtcojo_snp_rm(afsnps, _meta_snp_name_map, _meta_remain_snp);
    ...
    for (i = 0; i < npheno; i++) {
        ...
        n_o(i) = init_pc_meta(
            _meta_remain_snp, _meta_snp_freq.col(i), _meta_snp_b.col(i), _meta_snp_se.col(i), snp_n, _meta_vp_trait(i));
    ...
    for (i = 1; i < npheno; i++) {
        ...
        bxy_hat(i - 1) = est_bxy_pc(_meta_remain_snp,
                                    _meta_snp_b.col(0),
                                    _meta_snp_freq.col(0),
                                    bzx_tmp,
                                    _meta_snp_freq.col(i),
                                    _meta_snp_n_o.col(i),
                                    n_o(i),
                                    _ttl_mk_num,
                                    _eigen_value[i - 1]);
    ...
    restrict_snp_effect(_meta_snp_name_map,
                        _snp_name,
                        _include,
                        _meta_snp_freq,
                        _meta_snp_b,
