...
vector<vector<int>> split_extract(vector<int> extract) {
    ...
    for (i = 1; i < nExtract; i++) {
        if (extract[i] == indexToExtract[1] + 1)
            blockToExtract[nBlock][1] = extract[i];
        else {
...
void removeElement(eigenVector& vector, int indexToRemove) {
    ...
    if (indexToRemove < numElements)
        vector.segment(indexToRemove, numElements - indexToRemove) =
            vector.segment(indexToRemove + 1, numElements - indexToRemove);
...
vector<string> update_common_snps(const vector<string> snplist, vector<string> common_snps) {
    ...
    for (i = 0; i < nsnpbuf; i++) {
        ...
        if (iter != common_snps.end())
            snpbuf.push_back(snplist[i]);
    }
...
void read_metafile_list(string mtcojolist_file,
                        string& target_pheno,
                        string& target_pheno_file,
                        vector<string>& covar_pheno,
                        vector<string>& covar_pheno_file,
                        vector<double>& popu_prev,
                        vector<double>& smpl_prev) {
    ...
    target_pheno = line_elements[0];
    target_pheno_file = line_elements[1];
    // prevelance
    d_prev1 = nan("");
    d_prev2 = nan("");
    if (line_elements.size() == 4) {
        prevbuf1 = line_elements[2];
        prevbuf2 = line_elements[3];
        StrFunc::to_upper(prevbuf1);
        StrFunc::to_upper(prevbuf2);
    ...
    smpl_prev.push_back(d_prev1);
    popu_prev.push_back(d_prev2);
    ...
    while (std::getline(meta_list, strbuf)) {
        ...
        if (line_elements.size() != 2 && line_elements.size() != 4) {
            LOGGER.e(0,
                     "the format of file [" + mtcojolist_file + "] is incorrect, line " + to_string(line_number) + ".");
        }
...
void gcta::init_meta_snp_map(vector<string> snplist,
    ...
    for (i = 0; i < nsnp; i++) {
        ...
        if (size == snp_name_map.size())
            LOGGER.e(0, "Duplicated SNP ID found: " + snplist[i] + ".");
...
void gcta::init_gwas_variable(vector<vector<string>>& snp_a1,
                              vector<vector<string>>& snp_a2,
                              eigenMatrix& snp_freq,
                              eigenMatrix& snp_b,
                              eigenMatrix& snp_se,
                              eigenMatrix& snp_pval,
                              eigenMatrix& n,
                              int npheno,
                              int nsnp) {
    int i = 0;
    ...
    snp_a1.clear();
    snp_a2.clear();
    snp_a1.resize(npheno);
    snp_a2.resize(npheno);
    for (i = 0; i < npheno; i++) {
        snp_a1[i].resize(nsnp);
        snp_a2[i].resize(nsnp);
    }
    // allele frequency
    snp_freq.resize(nsnp, npheno);
    // effect size, se, p-value and sample size
    snp_b.resize(nsnp, npheno);
    snp_se.resize(nsnp, npheno);
    snp_pval.resize(nsnp, npheno);
    n.resize(nsnp, npheno);
}
...
void gcta::update_meta_snp_list(vector<string>& snplist, map<string, int> snp_id_map) {
    ...
    snplist.clear();
    for (i = 0; i < nsnpbuf; i++) {
        if (snp_id_map.find(snpbuf[i]) == snp_id_map.end())
            continue;
        snplist.push_back(snpbuf[i]);
    }
}

void gcta::update_meta_snp_map(vector<string> snplist,
                               map<string, int>& snp_id_map,
    ...
    for (i = 0, j = snp_id_map.size(); i < nsnp; i++) {
        ...
        if (size == snp_add_map.size())
            LOGGER.e(0, "Duplicated SNP ID found: " + snplist[i] + ".");
        size = snp_add_map.size();

        if (snp_id_map.find(snplist[i]) != snp_id_map.end())
            continue;
...
void gcta::update_meta_snp(map<string, int>& snp_name_map, vector<string>& snp_name, vector<int>& snp_remain) {
    ...
    snp_name.clear();
    snp_name.resize(nsnp);
    snp_remain.clear();
    snp_remain.resize(nsnp);
    for (i = 0; i < nsnp; i++) {
...
void update_mtcojo_snp_kp(const vector<string> adjsnps, map<string, int>& snp_id_map, vector<int>& remain_snp_indx) {
    ...
    for (i = 0; i < nsnpbuf; i++)
        snp_id_map_buf.erase(adjsnps[i]);
    ...
    remain_snp_indx.clear();
    remain_snp_indx.resize(nsnpbuf);
    for (iter = snp_id_map.begin(), i = 0; iter != snp_id_map.end(); iter++, i++)
        remain_snp_indx[i] = iter->second;
...
vector<string> gcta::read_snp_metafile_txt(string metafile, map<string, int>& gws_snp_name_map, double thresh) {
    ...
    while (std::getline(meta_snp, strbuf)) {
        ...
        if (line_elements.size() != 8) {
            LOGGER.e(0,
                     "the GWAS summary data file [" + metafile + "] should be in GCTA-COJO format, line " +
        ...
        double pval_buf = 1.0;
        if (valbuf != "." && valbuf != "NA" && valbuf != "NAN")
            pval_buf = atof(valbuf.c_str());
...
vector<string> gcta::read_snp_metafile_gz(string metafile, map<string, int>& gws_snp_name_map, double thresh) {
    ...
    while (1) {
        ...
        if (meta_snp.fail() || !meta_snp.good())
            break;
        line_number++;
        if (line_number == 1)
            continue;
        stringstream ss(buf);
        if (!(ss >> snpbuf))
            LOGGER.e(0, err_msg + to_string(line_number) + "," + buf);
        // Save the SNPs
        ...
        // Save the p-value
        double pval_buf = 1.0;
        if (strbuf != "." && strbuf != "NA" && strbuf != "NAN")
            pval_buf = atof(strbuf.c_str());
...
double gcta::read_single_metafile_txt(string metafile,
                                      map<string, int> id_map,
                                      vector<string>& snp_a1,
                                      vector<string>& snp_a2,
                                      eigenVector& snp_freq,
                                      eigenVector& snp_b,
                                      eigenVector& snp_se,
                                      eigenVector& snp_pval,
                                      eigenVector& snp_n,
                                      vector<bool>& snp_flag) {
    ...
    while (std::getline(meta_raw, strbuf)) {
        ...
        snp_a1[snp_indx] = line_elements[1];
        snp_a2[snp_indx] = line_elements[2];
        StrFunc::to_upper(snp_a1[snp_indx]);
        StrFunc::to_upper(snp_a2[snp_indx]);
        valbuf = line_elements[3];
        if (valbuf != "." && valbuf != "NA" && valbuf != "NAN")
            snp_freq(snp_indx) = atof(valbuf.c_str());
        else {
            snp_freq(snp_indx) = nan("");
            missing_flag = true;
        }
        valbuf = line_elements[4];
        if (valbuf != "." && valbuf != "NA" && valbuf != "NAN")
            snp_b(snp_indx) = atof(valbuf.c_str());
        else {
            snp_b(snp_indx) = nan("");
            missing_flag = true;
        }
        ...
        if (valbuf != "." && valbuf != "NA" && valbuf != "NAN")
            snp_se(snp_indx) = atof(valbuf.c_str());
        else {
            snp_se(snp_indx) = nan("");
            missing_flag = true;
        }
        valbuf = line_elements[6];
        if (valbuf != "." && valbuf != "NA" && valbuf != "NAN")
            snp_pval(snp_indx) = atof(valbuf.c_str());
        else {
            snp_pval(snp_indx) = nan("");
            missing_flag = true;
        }
        valbuf = line_elements[7];
        if (valbuf != "." && valbuf != "NA" && valbuf != "NAN")
            snp_n(snp_indx) = atof(valbuf.c_str());
        else {
            snp_n(snp_indx) = nan("");
            missing_flag = true;
        }
        ...
        if (!missing_flag) {
            ...
            vp_buf = h_buf * snp_b(snp_indx) * snp_b(snp_indx) +
                     h_buf * snp_n(snp_indx) * snp_se(snp_indx) * snp_se(snp_indx);
            vec_vp_buf.push_back(vp_buf);
        }
    }
    meta_raw.close();
    if (vec_vp_buf.size() > 0)
        median_vp = CommFunc::median(vec_vp_buf);

    return median_vp;
}

double gcta::read_single_metafile_gz(string metafile,
                                     map<string, int> id_map,
                                     vector<string>& snp_a1,
                                     vector<string>& snp_a2,
                                     eigenVector& snp_freq,
    ...
    while (1) {
        ...
        if (meta_raw.fail() || !meta_raw.good())
            break;
        missing_flag = false;
        line_number++;
        if (line_number == 1)
...
vector<string> gcta::remove_bad_snps(vector<string> snp_name,
                                     vector<int> snp_remain,
                                     vector<vector<bool>> snp_flag,
                                     vector<vector<string>>& snp_a1,
                                     vector<vector<string>>& snp_a2,
                                     eigenMatrix& snp_freq,
                                     eigenMatrix& snp_b,
                                     eigenMatrix snp_se,
                                     eigenMatrix snp_pval,
                                     eigenMatrix snp_n,
                                     const SnpNameMap& plink_snp_name_map,
                                     vector<string> snp_ref_a1,
                                     vector<string> snp_ref_a2,
                                     int ntarget,
                                     int ncovar,
                                     string outfile_name) {
    ...
    vector<int> bad_indx;
    SnpNameMap::const_iterator iter;
    ...
    for (i = 0; i < nsnp; i++) {
        ...
        if (iter != plink_snp_name_map.end())
            snpindx = iter->second;
        // Alleles
        if (snpindx > 0) {
            allelebuf.push_back(snp_ref_a1[snpindx]);
            allelebuf.push_back(snp_ref_a2[snpindx]);
        } else {
            for (j = 0; j < npheno; j++) {
        ...
        snp_a1_bak = allelebuf[0];
        snp_a2_bak = allelebuf[1];
        for (j = 0; j < npheno; j++) {
            if (!snp_flag[j][snp_remain[i]])
                continue;
            // Alleles
            ...
            if (std::isnan(snp_b(snp_remain[i], j)) || std::isnan(snp_se(snp_remain[i], j)) ||
                std::isnan(snp_pval(snp_remain[i], j)) || std::isnan(snp_n(snp_remain[i], j))) {
                iterFlag = false;
                break;
            }
    ...
    if (!badsnps.empty()) {
        ...
        if (!obadsnp)
            LOGGER.e(0, "cannot open file [" + badsnpfile + "] to write bad SNPs.");
        ...
        LOGGER.i(0,
                 to_string(nbadsnps) +
                     " SNPs have missing value or mismatched alleles. These SNPs have been saved in [" + badsnpfile +
                     "].");
    }
...
vector<string> gcta::remove_freq_diff_snps(vector<string> meta_snp_name,
                                           vector<int> meta_snp_remain,
                                           const SnpNameMap& snp_name_map,
                                           vector<double> ref_freq,
                                           eigenMatrix meta_freq,
                                           vector<vector<bool>> snp_flag,
                                           int ntrait,
                                           double freq_thresh,
                                           string outfile_name) {
    ...
    SnpNameMap::const_iterator iter_ref;
    ...
    for (i = 0; i < nsnp; i++) {
        ...
        int j = 0;
        for (j = 0; j < ntrait; j++) {
            if (!snp_flag[j][meta_snp_remain[i]])
                continue;
        ...
        for (t1 = 0; t1 < nsnp_test - 1; t1++) {
            for (t2 = t1 + 1; t2 < nsnp_test; t2++) {
                ...
                if (max_freq_dev < freq_dev)
                    max_freq_dev = freq_dev;
            }
        }
        if (max_freq_dev > freq_thresh)
    ...
    if (!afsnps.empty()) {
        ...
        if (!oafsnp)
            LOGGER.e(0, "cannot open file [" + afsnpfile + "] to write bad SNPs.");
        int nafsnps = afsnps.size();
        for (i = 0; i < nafsnps; i++)
            oafsnp << afsnps[i] << endl;
        oafsnp.close();
        LOGGER.i(0,
                 to_string(nafsnps) +
                     " SNP(s) have large difference of allele frequency between the GWAS summary data and the "
                     "reference sample. These SNPs have been saved in [" +
                     afsnpfile + "].");
        if (nafsnps > nsnp_ttl * 0.05)
            LOGGER.e(0,
                     "there are too many SNPs that have large difference in allele frequency. Please check the GWAS "
                     "summary data.");
    }
    return (afsnps);
}

vector<string> gcta::remove_mono_snps(const SnpNameMap& snp_name_map, vector<double> ref_snpfreq, string outfile_name) {
    int i = 0, n_raresnp = 0;
    vector<string> afsnps;
    SnpNameMap::const_iterator iter;
    ...
    for (iter = snp_name_map.begin(); iter != snp_name_map.end(); iter++) {
        ...
        if (af > 0.5)
            af = 1.0 - af;
        if (CommFunc::FloatEqual(af, 0.0))
            afsnps.push_back(iter->first);
        if (af < 0.01)
            n_raresnp++;
    }

    if (n_raresnp > 0)
        LOGGER.w(0, "There are " + to_string(n_raresnp) + " SNPs with MAF < 0.01 in the reference sample.");
...
vector<string> gcta::filter_meta_snp_pval(vector<string> snp_name,
                                          vector<int> remain_snp_indx,
                                          eigenMatrix snp_pval,
                                          int start_indx,
                                          int end_indx,
                                          vector<vector<bool>> snp_flag,
...
int gcta::read_mtcojofile(string mtcojolist_file, double gwas_thresh, int nsnp_gsmr) {
    ...
    _covar_pheno_name.clear();
    _meta_smpl_prev.clear();
    _meta_popu_prev.clear();
    read_metafile_list(mtcojolist_file,
                       _target_pheno_name,
                       target_pheno_file,
                       _covar_pheno_name,
                       covar_pheno_file,
    ...
    for (i = 0; i < ncovar; i++) {
        ...
        if (i == 0)
            init_meta_snp_map(snplist, _meta_snp_name_map, _meta_snp_name, _meta_remain_snp);
        else
            update_meta_snp_map(snplist, _meta_snp_name_map, _meta_snp_name, _meta_remain_snp, true);
    }
    ...
    init_gwas_variable(
        snp_a1, snp_a2, snp_freq, _meta_snp_b, _meta_snp_se, _meta_snp_pval, _meta_snp_n_o, ncovar + 1, nsnp);
    ...
    _snp_val_flag.clear();
    _snp_val_flag.resize(ncovar + 1);
    for (i = 0; i < ncovar + 1; i++) {
        _snp_val_flag[i].resize(nsnp);
        for (j = 0; j < nsnp; j++)
            _snp_val_flag[i][j] = false;
    }

    // Target trait
    if (target_pheno_file.substr(target_pheno_file.length() - 3, 3) != ".gz")
        _meta_vp_trait(0) = read_single_metafile_txt(target_pheno_file,
                                                     _meta_snp_name_map,
                                                     snp_a1[0],
                                                     snp_a2[0],
                                                     snp_freq_buf,
        ...
        _meta_vp_trait(0) = read_single_metafile_gz(target_pheno_file,
                                                    _meta_snp_name_map,
                                                    snp_a1[0],
                                                    snp_a2[0],
                                                    snp_freq_buf,
                                                    ...
                                                    snp_n_buf,
                                                    _snp_val_flag[0]);
    if (_meta_vp_trait(0) < 0)
        LOGGER.e(0, "negative phenotypic variance of the target trait, " + _covar_pheno_name[0] + ".");
    ...
    for (i = 0; i < ncovar; i++) {
        if (covar_pheno_file[i].substr(covar_pheno_file[i].length() - 3, 3) != ".gz")
            _meta_vp_trait(i + 1) = read_single_metafile_txt(covar_pheno_file[i],
                                                             _meta_snp_name_map,
                                                             snp_a1[i + 1],
                                                             snp_a2[i + 1],
                                                             snp_freq_buf,
            ...
            _meta_vp_trait(i + 1) = read_single_metafile_gz(covar_pheno_file[i],
                                                            _meta_snp_name_map,
                                                            snp_a1[i + 1],
                                                            snp_a2[i + 1],
                                                            snp_freq_buf,
                                                            ...
                                                            snp_n_buf,
                                                            _snp_val_flag[i + 1]);
        if (_meta_vp_trait(i + 1) < 0)
            LOGGER.e(0,
                     "negative phenotypic variance of the covariate #" + to_string(i + 1) + ", " +
                         _covar_pheno_name[i + 1] + ".");
    ...
    vector<string> badsnps;
    badsnps = remove_bad_snps(_meta_snp_name,
                              _meta_remain_snp,
                              _snp_val_flag,
                              snp_a1,
                              snp_a2,
    ...
    // For output
    _meta_snp_a1 = snp_a1[0];
    _meta_snp_a2 = snp_a2[0];
    _meta_snp_freq = snp_freq;
    nsnp = _meta_remain_snp.size();
    if (nsnp < 1)
        LOGGER.e(0, "no SNP is retained after filtering.");
    else
        LOGGER.i(0, to_string(nsnp) + " SNPs are retained after filtering.");

    // Only keep SNPs with p-value < threshold
    double pval_thresh = gwas_thresh;
    vector<string> keptsnps;

    keptsnps = filter_meta_snp_pval(
        _meta_snp_name, _meta_remain_snp, _meta_snp_pval, 1, 1 + ncovar, _snp_val_flag, pval_thresh);
    update_id_map_kp(keptsnps, _snp_name_map, _include);
    std::stringstream ss;
    ss << std::scientific << std::setprecision(1) << gwas_thresh;
    LOGGER.i(0,
             "There are " + to_string(_include.size()) + " genome-wide significant SNPs with p < " + ss.str() + ".\n");
    if (keptsnps.size() < nsnp_gsmr)
        LOGGER.w(0,
                 "We will use LD score regression to estimate bxy, because there are not enough significant SNPs for "
                 "GSMR analysis. At least " +
                     to_string(nsnp_gsmr) + " SNPs are required.\n");
...
eigenVector read_external_bxy(string filestr, vector<string> covar_pheno_name) {
    ...
    if (!extern_bxy)
        LOGGER.e(0, "cannot open the file [" + filestr + "] to read.");

    for (i = 0; i < ncovar; i++)
        covar_pheno_map.insert(pair<string, int>(covar_pheno_name[i], i));
...
vector<string> gcta::clumping_meta(
    eigenVector snp_chival, vector<bool> snp_flag, double pval_thresh, int wind_size, double r2_thresh) {
    ...
    for (i = 0; i < nsnp_clumped; i++) {
        ...
        while (1) {
            ...
            if (_chr[geno_indx] == _chr[_include[geno_indx_j]] &&
                abs(_bp[geno_indx] - _bp[_include[geno_indx_j]]) < wind_size) {
                iter_clump = clumped_snp.find(snpbuf_left);
                if (iter_clump == clumped_snp.end())
                    continue;
                // r2
                ...
                // save the SNP
                if (r2_left >= r2_thresh)
                    iter_clump->second = true;
            } else {
                break;
            }
        }
...
vector<int> rm_cor_elements(eigenMatrix r_mat, double r2_thresh, bool r2_flag) {
    ...
    if (rm_mat_ID1.size() > 0) {
        ...
        for (i = 0; i < rm_uni_ID.size(); i++) {
            i_buf = count(rm_mat_ID1.begin(), rm_mat_ID1.end(), rm_uni_ID[i]) +
                    count(rm_mat_ID2.begin(), rm_mat_ID2.end(), rm_uni_ID[i]);
...
void adjust_ld_r_fdr(
    eigenMatrix& ld_r_mat, vector<int> kept_ID, vector<pair<double, int>> ld_pval, int m, double thresh) {
    int i = 0, nproc = ld_pval.size(), row_indx = 0, col_indx = 0;
    vector<double> pval_buf(nproc);

    for (i = 0; i < nproc; i++)
        pval_buf[i] = ld_pval[i].first;

    pval_buf = StatFunc::ControlFDR_BH(pval_buf);

    for (i = 0; i < nproc; i++) {
        if (pval_buf[i] < thresh)
            break;
...
vector<int>
init_interval_bxy(eigenVector bxy_sort, eigenVector bxy, vector<int> kept_ID, double lower_prob, double upper_prob) {
...
int topsnp_bxy(eigenVector bxy,
               eigenVector bzx,
               eigenVector bzx_se,
               map<string, int> meta_snp_name_map,
               vector<string> indices_snp,
               vector<int> kept_ID,
               int n_indices_snp) {
    // find the top SNP
...
void est_cov_bxy(eigenMatrix& cov_bxy_p1,
                 eigenMatrix& cov_bxy_p2,
                 eigenVector bzx,
                 eigenVector bzx_se,
                 eigenVector bzy_se,
                 eigenMatrix ld_r_mat,
                 map<string, int> meta_snp_name_map,
                 vector<string> indices_snp,
                 vector<int> kept_ID) {
...
void est_cov_bxy_gsmr(eigenMatrix& cov_bxy,
                      double bxy_hat,
                      eigenVector bzx,
                      eigenVector bzx_se,
                      eigenVector bzy_se,
    ...
    est_cov_bxy(cov_bxy_p1, cov_bxy_p2, bzx, bzx_se, bzy_se, ld_r_mat, meta_snp_name_map, indices_snp, kept_ID);
    cov_bxy = cov_bxy_p1 + cov_bxy_p2 * (bxy_hat * bxy_hat);
}

void est_cov_bxy_heidi(eigenMatrix& cov_bxy,
                       double bxy_hat,
                       eigenVector bzx,
                       eigenVector bzx_se,
                       eigenVector bzy,
...
vector<double> est_bxy_gsmr(eigenVector bxy_sort,
                            eigenVector bxy,
                            vector<string> indices_snp,
                            vector<int> kept_ID,
                            eigenVector bzx,
    ...
    est_cov_bxy_gsmr(cov_bxy, bxy_median, bzx, bzx_se, bzy_se, ld_r_mat, meta_snp_name_map, indices_snp, kept_ID);

    eigenVector bxy_kept(n_indices_snp);
    for (i = 0; i < n_indices_snp; i++)
        bxy_kept(i) = bxy(kept_ID[i]);
    ...
    rst[1] = bxy_gsmr_se;
    rst[2] = bxy_gsmr_pval;
    rst[3] = (double)n_indices_snp;
    return rst;
}

void est_var_bxy(eigenVector& var_bxy,
                 eigenVector bzx,
                 eigenVector bzx_se,
                 eigenVector bzy,
                 eigenVector bzy_se,
...
#pragma omp parallel for
    for (int i = 0; i < n_indices_snp; i++) {
        map<string, int>::iterator iter;
        iter = meta_snp_name_map.find(indices_snp[kept_ID[i]]);
        double d_buf = pow(bzx_se(iter->second), 2.0) * pow(bzy(iter->second), 2.0) / pow(bzx(iter->second), 4.0) +
                       pow(bzy_se(iter->second), 2.0) / pow(bzx(iter->second), 2.0);
        var_bxy(i) = d_buf;
    }
}

void est_diff_bxy(eigenVector& bxy_diff,
                  double bxy_est,
                  vector<string> indices_snp,
                  vector<int> kept_ID,
                  eigenVector bzx,
...
void est_var_diff_bxy(
    eigenMatrix& var_d, eigenMatrix cov_bxy, eigenVector cov_bxy_bgsmr, double bxy_hat_se, int n_indices_snp) {
...
vector<double> indi_heidi_pvalue(double bxy_hat,
                                 double bxy_hat_se,
                                 eigenVector vec_1t_v,
                                 eigenVector bzx,
                                 eigenVector bzx_se,
...
double global_heidi_pvalue(double bxy_hat,
                           double bxy_hat_se,
                           eigenVector bzx,
                           eigenVector bzx_se,
                           eigenVector bzy,
...
#pragma omp parallel for
    for (int i = 0; i < n_indices_snp; i++)
        cov_bxy_bgsmr(i) = bxy_hat_se * bxy_hat_se * vec_1t_v.dot(cov_bxy.col(i));
...
vector<int> indi_heidi_outlier(eigenVector bxy,
                               eigenVector bzx,
                               eigenVector bzx_se,
                               eigenVector bzy,
                               eigenVector bzy_se,
    ...
    eigenVector vec_1t_v;
    vector<double> gsmr_rst = est_bxy_gsmr(
        bxy_sort, bxy, indices_snp, ci_index, bzx, bzx_se, bzy, bzy_se, ld_r_mat, meta_snp_name_map, vec_1t_v);
    double bxy_hat = gsmr_rst[0], bxy_hat_se = gsmr_rst[1];

    vector<double> indi_het_pval;
    indi_het_pval = indi_heidi_pvalue(bxy_hat,
                                      bxy_hat_se,
                                      vec_1t_v,
                                      bzx,
                                      bzx_se,
...
vector<int> indi_heidi_outlier_iter(eigenVector bxy,
                                    eigenVector bzx,
                                    eigenVector bzx_se,
                                    eigenVector bzy,
                                    eigenVector bzy_se,
    ...
    if (min_pval < indi_heidi_thresh)
        kept_ID.erase(kept_ID.begin() + min_index);

    return (kept_ID);
}

int topsnp_heidi_outlier(eigenVector bxy,
                         vector<int> kept_ID,
                         vector<int> ci_index,
                         eigenVector bzx,
                         eigenVector bzx_se,
    ...
    for (int i = 0; i < n_candidate_snp; i++)
        bxy_sort(i) = bxy(kept_ID[ci_index[i]]);
    ...
    for (int i = 0; i < n_candidate_snp; i++) {
        for (int j = 0; j < nbins; j++) {
            if (bxy(kept_ID[ci_index[i]]) < lower_bounder[j] || bxy(kept_ID[ci_index[i]]) >= upper_bounder[j])
                continue;
...
double heidi_outlier_topsnp(eigenVector& indi_het_pval,
                            eigenVector bxy,
                            eigenMatrix cov_bxy,
                            vector<int> kept_ID,
                            eigenVector bzx,
    ...
    for (int i = 0; i < n_indices_snp; i++)
        var_d.col(i) -= cov_bxy.col(topsnp_index);
    for (int i = 0; i < n_indices_snp; i++)
        var_d.row(i) -= cov_bxy.col(topsnp_index);
    ...
    if (!multi_snp_heidi_flag) {
        ...
        removeRow(var_d, topsnp_index);
        removeColumn(var_d, topsnp_index);
        // Saddle point method
...
vector<int> keep_non_associate_snp(eigenVector bzy_pval,
                                   vector<string> indices_snp,
                                   vector<int> kept_ID,
                                   map<string, int> meta_snp_name_map,
                                   double pval_thresh) {
    ...
    for (int i = 0; i < n_indices_snp; i++) {
        map<string, int>::iterator iter = meta_snp_name_map.find(indices_snp[kept_ID[i]]);
        if (bzy_pval(iter->second) < pval_thresh)
            continue;
...
int indi_heidi_outlier_topsnp_iter(eigenVector bxy,
                                   eigenMatrix& cov_bxy,
                                   eigenVector bzx,
                                   eigenVector bzx_se,
                                   eigenVector bzy,
                                   eigenVector bzy_se,
                                   eigenVector bzy_pval,
                                   eigenMatrix ld_r_mat,
                                   map<string, int> meta_snp_name_map,
                                   vector<string> indices_snp,
                                   vector<int>& kept_ID,
    ...
    heidi_outlier_topsnp(
        indi_het_pval, bxy, cov_bxy, kept_ID, bzx, bzy, topsnp_index, indices_snp, meta_snp_name_map, false);
...
double global_heidi_outlier_topsnp_iter(eigenVector bxy,
                                        eigenMatrix& cov_bxy,
                                        eigenVector bzx,
                                        eigenVector bzx_se,
                                        eigenVector bzy,
    ...
    eigenVector indi_het_pval(n_indices_snp);
    global_heidi_pvalue = heidi_outlier_topsnp(
        indi_het_pval, bxy, cov_bxy, kept_ID, bzx, bzy, topsnp_index, indices_snp, meta_snp_name_map, true);
...
vector<double> gcta::gsmr_meta(vector<string>& snp_instru,
                               eigenVector bzx,
                               eigenVector bzx_se,
                               eigenVector bzx_pval,
                               eigenVector bzy,
                               eigenVector bzy_se,
                               eigenVector bzy_pval,
                               double rho_pheno,
                               ...
                               double global_heidi_thresh,
                               double ld_fdr_thresh,
                               int nsnp_gsmr,
                               string& pleio_snps,
                               string& err_msg) {
    ...
    for (i = 0; i < _n_gsmr_rst_item; i++)
        rst[i] = nan("");

    if (nsnp < nsnp_gsmr) {
        err_msg =
            "Not enough SNPs to perform the GSMR analysis. At least " + to_string(nsnp_gsmr) + " SNPs are required.";
        return rst;
    }
    ...
    // LOGGER.i(0, to_string(n_indices_snp) + " index SNPs are obtained from the clumping analysis of the " + nsnp + "
    ...
    if (n_indices_snp < nsnp_gsmr) {
        LOGGER.i(0,
                 to_string(n_indices_snp) + " index SNPs are obtained from the clumping analysis with p < " +
                     ss1.str() + " and LD r2 < " + ss2.str() + ".");
        err_msg =
            "Not enough SNPs to perform the GSMR analysis. At least " + to_string(nsnp_gsmr) + " SNPs are required.";
        return rst;
    }
    ...
    LOGGER.i(0,
             to_string(n_indices_snp) + " index SNPs are obtained from the clumping analysis with p < " + ss1.str() +
                 " and LD r2 < " + ss2.str() + ".");
    ...
    for (i = 0, k = 0; i < (n_indices_snp - 1); i++) {
        for (j = (i + 1); j < n_indices_snp; j++) {
            ld_pval[k++] =
                make_pair(StatFunc::chi_prob(
                              1, ld_r_mat(kept_ID[i], kept_ID[j]) * ld_r_mat(kept_ID[i], kept_ID[j]) * (double)nindi),
                          i * n_indices_snp + j);
        }
    }

    stable_sort(ld_pval.begin(), ld_pval.end(), [](const pair<double, int> a, const pair<double, int> b) {
    ...
    bxy.setZero();
    for (i = 0; i < n_indices_snp; i++) {
        auto meta_iter = _meta_snp_name_map.find(indices_snp[kept_ID[i]]);
        bxy(kept_ID[i]) = bzy(meta_iter->second) / bzx(meta_iter->second);
    }
    ...
    if (_gsmr_beta_version & heidi_flag) {
        ...
        while (1) {
            ...
            int process = indi_heidi_outlier_topsnp_iter(bxy,
                                                         cov_bxy,
                                                         bzx,
                                                         bzx_se,
                                                         bzy,
                                                         ...
                                                         std_heidi_thresh);
            n_indices_snp = kept_ID.size();
            if (n_kept_snp - n_indices_snp == 0)
                break;
            // check
            if (n_indices_snp < nsnp_gsmr || process == 0) {
                err_msg = "Not enough SNPs to perform GSMR analysis. At least " + to_string(nsnp_gsmr) +
                          " SNPs are required.";
                return rst;
            }
        }
        ...
        while (1) {
            // bxy by GSMR
            n_kept_snp = kept_ID.size();
            global_heidi_pval = global_heidi_outlier_topsnp_iter(bxy,
                                                                 cov_bxy,
                                                                 bzx,
                                                                 bzx_se,
                                                                 bzy,
        ...
        if (topsnp_index < 0) {
            err_msg = "Not enough SNPs to perform the GSMR analysis. At least " + to_string(nsnp_gsmr) +
                      " SNPs are required.";
            return rst;
        }
        ...
        eigenVector indi_het_pval(n_indices_snp);
        heidi_outlier_topsnp(
            indi_het_pval, bxy, cov_bxy, kept_ID, bzx, bzy, topsnp_index, indices_snp, _meta_snp_name_map, false);
        ...
        for (i = 0; i < n_indices_snp; i++) {
            ...
            if (indi_het_pval(i) >= std_heidi_thresh)
                kept_ID_tmp.push_back(kept_ID[i]);
        }
        kept_ID = kept_ID_tmp;
        n_indices_snp = kept_ID.size();
    ...
    eigenVector vec_1t_v;
    vector<double> gsmr_rst = est_bxy_gsmr(
        bxy_kept, bxy, indices_snp, kept_ID, bzx, bzx_se, bzy, bzy_se, ld_r_mat, _meta_snp_name_map, vec_1t_v);
    rst[0] = gsmr_rst[0];
    rst[1] = gsmr_rst[1];
    rst[2] = StatFunc::pchisq(rst[0] * rst[0] / (rst[1] * rst[1]), 1);
    rst[3] = n_indices_snp;
    ...
    snp_instru.clear();
    snp_instru.resize(n_indices_snp);
    for (i = 0; i < n_indices_snp; i++)
...
int read_ld_marker(string ref_ld_dirt) {
    ...
    for (i = 0; i < chr_num; i++) {
        ...
        if (line_elements.size() != 1)
            LOGGER.e(0, "the format of file [" + filestr + "] is incorrect.");
...
vector<string> read_ld_score_gz(string filestr, map<string, int> snplist_map, vector<double>& ld_score) {
    ...
    while (1) {
        ...
        if (ldsc_marker.fail() || !ldsc_marker.good())
            break;
        line_number++;
        std::istringstream linebuf(strbuf);
        std::istream_iterator<string> begin(linebuf), end;
...
vector<string> read_ld_score(string ld_dirt, map<string, int> snplist_map, int nsnp, vector<double>& ld_score) {
    ...
    for (i = 0; i < chr_num; i++) {
        ...
        if (file_exists(filestr_t1)) {
            ...
            LOGGER.e(0, "cannot open the file [" + filestr_t1 + "] or [" + filestr_t2 + "] to read.");
    }

    return ld_score_snps;
}

eigenVector update_weights_hsq(
    double intercept, double h, int ttl_mk_num, int n_ld_snp, eigenVector ref_ld, eigenVector w_ld, eigenVector n) {
    int i = 0;
    if (h < 0.0)
        h = 0.0;
    if (h > 1.0)
...
eigenVector compute_irls(double& intercept,
                         double& hsq,
                         eigenMatrix x,
                         eigenVector y,
                         eigenVector wt,
...
vector<double> est_hsq_trait_1_step(
    eigenVector chival, eigenVector n, eigenVector ref_ld, eigenVector w_ld, int n_ld_snp, int ttl_mk_num) {
    ...
    x.col(1).setOnes();
    x.col(0) = ref_ld.cwiseProduct(n) / n.mean();

    // Iteratively reweighted least squares
    wt_ttl =
        compute_irls(intercept_prior, h_prior, x, chival, wt_ttl, ref_ld, w_ld, n, ttl_mk_num, n_ld_snp, true, false);
    wt_ttl =
        compute_irls(intercept_prior, h_prior, x, chival, wt_ttl, ref_ld, w_ld, n, ttl_mk_num, n_ld_snp, true, false);
    // Estimate intercept and slope
    double intercept_posterior = 0.0, h_posterior = 0.0;
    wt_ttl = compute_irls(
        intercept_posterior, h_posterior, x, chival, wt_ttl, ref_ld, w_ld, n, ttl_mk_num, n_ld_snp, true, false);

    vector<double> ldsc_est(2);
    ldsc_est[0] = intercept_posterior;
    ldsc_est[1] = h_posterior;
    return ldsc_est;
}

vector<double> est_hsq_trait_2_steps(
    eigenVector chival, eigenVector n, eigenVector ref_ld, eigenVector w_ld, int n_ld_snp, int ttl_mk_num) {
    ...
    for (i = 0; i < n_ld_snp; i++) {
        if (chival(i) < thresh)
            subset_indx.push_back(i);
    }
    n_subset_snp = subset_indx.size();

    subset_chi.resize(n_subset_snp);
    subset_n.resize(n_subset_snp);
    subset_ref_ld.resize(n_subset_snp);
    subset_w_ld.resize(n_subset_snp);
    subset_wt.resize(n_subset_snp);
    subset_x.resize(n_subset_snp, 2);
    ...
    // Iteratively reweighted least squares
    subset_wt = compute_irls(intercept_prior,
                             h_prior,
                             subset_x,
                             subset_chi,
                             subset_wt,
    ...
    subset_wt = compute_irls(intercept_posterior,
                             h_prior,
                             subset_x,
                             subset_chi,
                             subset_wt,
    ...
    wt_ttl = compute_irls(
        intercept_posterior, h_prior, x, chival, wt_ttl, ref_ld, w_ld, n, ttl_mk_num, n_ld_snp, false, false);
    wt_ttl = compute_irls(
        intercept_posterior, h_prior, x, chival, wt_ttl, ref_ld, w_ld, n, ttl_mk_num, n_ld_snp, false, false);
    ...
    wt_ttl = compute_irls(
        intercept_posterior, h_posterior, x, chival, wt_ttl, ref_ld, w_ld, n, ttl_mk_num, n_ld_snp, false, false);
...
eigenVector update_weights_gcov(double intercept1,
                                double h1,
                                double intercept2,
                                double h2,
                                double intercept_gcov,
                                double gcov,
                                int ttl_mk_num,
                                int n_ld_snp,
                                eigenVector ref_ld,
...
eigenVector compute_irls_gcov(double& intercept_gcov,
                              double& gcov,
                              eigenMatrix x,
                              eigenVector y,
                              eigenVector wt,
    ...
    // weights
    if (x_flag) {
        w = update_weights_gcov(intercept1,
                                hsq1,
                                intercept2,
                                hsq2,
                                intercept_gcov,
...
vector<double> est_gcov_trait_1_step(eigenVector zscore,
                                     eigenVector n_gcov,
                                     eigenVector ref_ld,
                                     eigenVector w_ld,
                                     double intercept1,
    ...
    eigenVector wt_ttl = update_weights_gcov(intercept1,
                                             hsq1,
                                             intercept2,
                                             hsq2,
                                             intercept_gcov_prior,
                                             gcov_prior,
                                             ttl_mk_num,
                                             n_ld_snp,
                                             ref_ld,
    ...
    eigenMatrix x(n_ld_snp, 2);
    x.col(1).setOnes();
    x.col(0) = ref_ld.cwiseProduct(n_gcov) / n_gcov.mean();
    ...
    wt_ttl = compute_irls_gcov(intercept_gcov_posterior,
                               gcov_posterior,
                               x,
                               zscore,
                               wt_ttl,
    ...
    vector<double> ldsc_est(2);
    ldsc_est[0] = intercept_gcov_posterior;
    ldsc_est[1] = gcov_posterior / sqrt(hsq1 * hsq2);
    return ldsc_est;
}
...
vector<string> gcta::read_snp_ldsc(map<string, int> ldsc_snp_name_map,
                                   vector<string> snp_name,
                                   vector<int> snp_remain,
                                   int& ttl_mk_num,
                                   string ref_ld_dirt,
                                   string w_ld_dirt,
                                   vector<double>& ref_ld_vec,
                                   vector<double>& w_ld_vec) {
    int i = 0, nsnp = snp_remain.size();
...
void gcta::reorder_snp_effect(vector<int> snp_remain,
                              eigenMatrix& bhat_z,
                              eigenMatrix& bhat_n,
                              eigenMatrix snp_b,
                              eigenMatrix snp_se,
                              eigenMatrix snp_n,
                              vector<vector<bool>>& snp_flag,
                              vector<vector<bool>> snp_val_flag,
                              vector<int>& nsnp_cm_trait,
                              vector<string> cm_ld_snps,
    ...
    ref_ld.setZero(n_cm_ld_snps);
    w_ld.resize(n_cm_ld_snps);
    w_ld.setZero(n_cm_ld_snps);
    bhat_z.resize(n_cm_ld_snps, ntrait);
    bhat_z.setZero(n_cm_ld_snps, ntrait);
    bhat_n.resize(n_cm_ld_snps, ntrait);
    bhat_n.setZero(n_cm_ld_snps, ntrait);
    snp_flag.clear();
    snp_flag.resize(ntrait);
    for (i = 0; i < ntrait; i++)
        snp_flag[i].resize(n_cm_ld_snps);
    for (i = 0; i < n_cm_ld_snps; i++) {
        iter = ldsc_snp_name_map.find(cm_ld_snps[i]);
        indxbuf = iter->second;
        ref_ld(i) = ref_ld_vec[indxbuf];
        w_ld(i) = w_ld_vec[indxbuf];
        for (j = 0; j < ntrait; j++) {
...
eigenMatrix gcta::ldsc_snp_h2(eigenMatrix bhat_z,
                              eigenMatrix bhat_n,
                              eigenVector ref_ld,
                              eigenVector w_ld,
                              vector<vector<bool>> snp_flag,
    ...
    for (i = 0; i < ntrait; i++) {
        ...
        if (nsnp_cm_trait[i] < nsnp_ldsc_thresh)
            LOGGER.w(0,
                     "Only " + to_string(nsnp_cm_trait[i]) +
                         " are retained in the univariate LD score regression analysis for " + trait_name[i] +
                         ". The estimate may not be accurate.");
        int k = 0, nsnp_buf = 0;
        eigenVector chi_val_buf(nsnp_cm_trait[i]), n_buf(nsnp_cm_trait[i]), ref_ld_buf(nsnp_cm_trait[i]),
            w_ld_buf(nsnp_cm_trait[i]);
...
eigenMatrix gcta::ldsc_snp_rg(eigenMatrix ldsc_var_h2,
                              eigenMatrix bhat_z,
                              eigenMatrix bhat_n,
                              eigenVector ref_ld,
                              eigenVector w_ld,
                              vector<vector<bool>> snp_flag,
                              vector<int> trait_indx1,
                              vector<int> trait_indx2,
                              int n_cm_ld_snps,
                              int ttl_mk_num,
                              vector<string> trait_name) {
    // Estimate SNP rg
    ...
    for (i = 0; i < nproc; i++) {
        ...
        if (n_cm_snps_buf < nsnp_ldsc_thresh)
            LOGGER.w(0,
                     "Only " + to_string(n_cm_snps_buf) +
                         " are retained in the bivariate LD score regression analysis for " +
                         trait_name[trait_indx1[i]] + " and " + trait_name[trait_indx2[i]] +
                         ". The estimate may not be accurate.");
        eigenVector gcov_z1z2(n_cm_snps_buf), gcov_n1n2(n_cm_snps_buf), ref_ld_buf(n_cm_snps_buf),
            w_ld_buf(n_cm_snps_buf), n_buf_i(n_cm_snps_buf), n_buf_j(n_cm_snps_buf);
        int t = 0;
        ...
        rst_ldsc = est_gcov_trait_1_step(gcov_z1z2,
                                         gcov_n1n2,
                                         ref_ld_buf,
                                         w_ld_buf,
                                         ldsc_var_h2(trait_indx1[i], 0),
                                         ldsc_var_h2(trait_indx1[i], 1),
                                         n_buf_i,
                                         ldsc_var_h2(trait_indx2[i], 0),
                                         ldsc_var_h2(trait_indx2[i], 1),
                                         n_buf_j,
                                         n_cm_snps_buf,
                                         ttl_mk_num);
...
bool gcta::mtcojo_ldsc(vector<vector<bool>> snp_val_flag,
                       eigenMatrix snp_b,
                       eigenMatrix snp_se,
                       eigenMatrix snp_n,
                       int ntrait,
                       ...
                       string w_ld_dirt,
                       vector<string> trait_name,
                       eigenMatrix& ldsc_intercept,
                       eigenMatrix& ldsc_slope) {
    ...
    for (i = 0; i < ntrait; i++)
        nsnp_cm_trait[i] = 0;
    // Read the LD scores
    cm_ld_snps = read_snp_ldsc(
        ldsc_snp_name_map, snp_name, snp_remain, ttl_mk_num, ref_ld_dirt, w_ld_dirt, ref_ld_vec, w_ld_vec);
    // Re-order the SNP effects and LD scores
    reorder_snp_effect(snp_remain,
                       bhat_z,
                       bhat_n,
                       snp_b,
                       snp_se,
    ...
    ldsc_var_h2 = ldsc_snp_h2(
        bhat_z, bhat_n, ref_ld, w_ld, snp_flag, nsnp_cm_trait, n_cm_ld_snps, ttl_mk_num, trait_name, ntrait);
    for (i = 0; i < ntrait; i++) {
    ...
    ldsc_var_rg = ldsc_snp_rg(ldsc_var_h2,
                              bhat_z,
                              bhat_n,
                              ref_ld,
                              w_ld,
                              ...
                              trait_name);
    for (i = 0; i < nproc; i++) {
        ldsc_intercept(trait_indx1[i], trait_indx2[i]) = ldsc_intercept(trait_indx2[i], trait_indx1[i]) =
            ldsc_var_rg(i, 0);
...
eigenMatrix mtcojo_cond_single_covar(eigenVector bzy,
                                     eigenVector bzy_se,
                                     eigenMatrix bzx,
                                     eigenMatrix bzx_se,
                                     double bxy,
    ...
    for (i = 0; i < nsnp; i++) {
        ...
        mtcojo_est(i, 2) =
            StatFunc::pchisq(mtcojo_est(i, 0) * mtcojo_est(i, 0) / mtcojo_est(i, 1) / mtcojo_est(i, 1), 1);
    }

    return mtcojo_est;
}

eigenMatrix mtcojo_cond_multiple_covars(eigenVector bzy,
                                        eigenVector bzy_se,
                                        eigenMatrix bzx,
                                        eigenMatrix bzx_se,
                                        eigenVector bxy,
...
void mtcojo_cond_output(string output_file,
                        vector<string> snp_name,
                        vector<int> snp_remain,
                        vector<string> snp_a1,
                        vector<string> snp_a2,
    ...
    if (!ofile)
        LOGGER.e(0, "cannot open the file [" + output_file + "] to write.");
    ...
    for (i = 0; i < nsnp; i++) {
        ...
        ofile << snp_name[snpindx] << "\t" << snp_a1[snpindx] << "\t" << snp_a2[snpindx] << "\t" << snp_freq[snpindx]
              ...
              << snp_n[snpindx] << "\t" << mtcojo_est(i, 0) << "\t" << mtcojo_est(i, 1) << "\t" << mtcojo_est(i, 2)
              << "\t" << endl;
    }
    ofile.close();
}

void gcta::mtcojo(string mtcojo_bxy_file,
                  string ref_ld_dirt,
                  string w_ld_dirt,
                  double freq_thresh,
                  double gwas_thresh,
                  int clump_wind_size,
                  double clump_r2_thresh,
                  double std_heidi_thresh,
                  double global_heidi_thresh,
                  double ld_fdr_thresh,
                  int nsnp_gsmr,
                  int gsmr_beta_version) {
    if (gsmr_beta_version) {
        _n_gsmr_rst_item = 5;
        _gsmr_beta_version = 1;
    } else {
        _n_gsmr_rst_item = 4;
        _gsmr_beta_version = 0;
    }
    ...
    // Read the bxy
    if (!mtcojo_bxy_file.empty())
        bxy_est = read_external_bxy(mtcojo_bxy_file, _covar_pheno_name);
    ...
    afsnps = remove_freq_diff_snps(_meta_snp_name,
                                   _meta_remain_snp,
                                   _snp_name_map,
                                   _mu,
                                   _meta_snp_freq,
    ...
    bool proc_flag = true;
    proc_flag = mtcojo_ldsc(_snp_val_flag,
                            _meta_snp_b,
                            _meta_snp_se,
                            _meta_snp_n_o,
                            ncovar + 1,
    ...
    if (!proc_flag) {
        if (ncovar == 1) {
            ...
            LOGGER.e(
                0,
                "not enough SNPs to perform the univariate LD score regression analysis. The mtCOJO analysis will be "
                "conducted assuming no sample overlap between the GWAS data for the target and covariate traits.");
        } else {
            LOGGER.e(0,
                     "not enough SNPs to perform the univariate and bivariate LD score regression analyses. The mtCOJO "
                     "analysis requires SNP-based heritability from univariate LD score regression analysis and "
                     "genetic correlation from bivariate LD score regression analysis.");
        }
    }
    ...
    for (i = 1; i <= ncovar; i++) {
        ...
        for (j = 0; j < nsnp; j++)
            snp_pair_flag[j] = _snp_val_flag[0][_meta_remain_snp[j]] && _snp_val_flag[i][_meta_remain_snp[j]];
        gsmr_rst = gsmr_meta(snp_instru,
                             _meta_snp_b.col(i),
                             _meta_snp_se.col(i),
                             _meta_snp_pval.col(i),
                             _meta_snp_b.col(0),
                             _meta_snp_se.col(0),
                             _meta_snp_pval.col(0),
                             0,
                             snp_pair_flag,
                             gwas_thresh,
                             clump_wind_size,
    ...
    if (pleio_flag) {
        ...
        if (!o_pleio_snp)
            LOGGER.e(0, "cannot open file [" + pleio_snpfile + "] to write pleiotropic SNPs.");
        o_pleio_snp << ss_pleio.str();
        o_pleio_snp.close();
        LOGGER.i(0,
                 "The pleiotropic SNPs filtered by HEIDI-outlier analysis have been saved in [" + pleio_snpfile + "].");
    }
    ...
    for (i = 0; i < nsnp; i++) {
        n_buf = 0;
        for (j = 0; j < ncovar + 1; j++)
            n_buf += _snp_val_flag[j][_meta_remain_snp[i]];
        if (n_buf < ncovar + 1)
            continue;
    ...
    LOGGER.i(0, "\nmtCOJO analysis ...");
    LOGGER.i(
        0, "There are " + to_string(nsnp) + " SNPs in common between the target trait and all the covariate trait(s).");
    if (ncovar == 1) {
        mtcojo_est = mtcojo_cond_single_covar(
            snp_bzy, snp_bzy_se, snp_bzx, snp_bzx_se, bxy_est(0), ldsc_intercept, ldsc_slope, nsnp);
    } else {
        mtcojo_est = mtcojo_cond_multiple_covars(snp_bzy,
                                                 snp_bzy_se,
                                                 snp_bzx,
                                                 snp_bzx_se,
                                                 bxy_est,
    ...
    LOGGER.i(0,
             "Saving the mtCOJO analysis results of " + to_string(nsnp) + " remaining SNPs to [" + output_filename +
                 "] ...");
    LOGGER.i(0, "mtCOJO analysis completed.");
    mtcojo_cond_output(output_filename,
                       _meta_snp_name,
                       _meta_remain_snp,
                       _meta_snp_a1,
                       _meta_snp_a2,
