...
namespace {
using CojoClock = std::chrono::steady_clock;

inline long long cojo_ns_since(CojoClock::time_point t0) {
    return std::chrono::duration_cast<std::chrono::nanoseconds>(CojoClock::now() - t0).count();
}
} // namespace
...
bool inv_update_forward_append(const eigenMatrix& R_inv, const eigenVector& c, double d, eigenMatrix& R_inv_out) {
...
void inv_update_backward(const eigenMatrix& R_inv, int pos, eigenMatrix& R_inv_out) {
    ...
    eigenMatrix tmp = R_inv - R_inv.col(pos) * R_inv.row(pos) / R_inv(pos, pos);
...
void gcta::prepare_deferred_geno_for_cojo(const string& metafile) {
    if (!_deferred_geno_load)
        return;
    if (_deferred_bedfile.empty())
        LOGGER.e(0, "internal error: deferred BED path is empty.");
    ...
    if (StrFunc::split_string(str_buf, vs_buf) < 7)
        LOGGER.e(0, "format error in the input file [" + metafile + "].");
    ...
    if (candidates.empty())
        LOGGER.e(0, "none of the SNPs in the GWAS summary data can be found in the genotype data.");

    update_id_map_kp(candidates, _snp_name_map, _include);
    LOGGER << "Deferred COJO: loading genotypes for " << _include.size() << " panel SNPs from [" << _deferred_bedfile
           << "] ..." << endl;
    read_bedfile(_deferred_bedfile);

    if (_deferred_maf > 0)
        filter_snp_maf(_deferred_maf);
    if (_deferred_max_maf > 0.0)
        filter_snp_max_maf(_deferred_max_maf);
...
void gcta::read_metafile(string metafile, bool GC, double GC_val) {
    ...
    if (_bim_region_filter) {
        LOGGER << "BIM region filter active (chr=" << _bim_region_chr << " bp=[" << _bim_region_start << ","
               << _bim_region_end << "]): matching .ma SNPs to region index only." << endl;
    }
    ifstream Meta(metafile.c_str());
    if (!Meta)
        LOGGER.e(0, "cannot open the file [" + metafile + "] to read.");
    ...
    int i = 0, count = 0;
    double f_buf = 0.0, b_buf = 0.0, se_buf = 0.0, p_buf = 0.0, N_buf = 0.0, Vp_buf = 0.0, GC_buf = 0.0, chi_buf = 0.0,
           h_buf = 0.0;
    ...
    while (Meta) {
        ...
        if (_bim_region_filter && !in_region_index) {
            ...
            iss >> str_buf;
            if (str_buf == "NA" || str_buf == ".") {
                skipped_ma++;
                continue;
            }
            b_buf = atof(str_buf.c_str());
            iss >> str_buf;
            if (str_buf == "NA" || str_buf == "." || str_buf == "0") {
                skipped_ma++;
                continue;
            }
            ...
            if (Meta.eof())
                break;
            if (N_buf < 10)
                LOGGER.e(0, "invalid sample size in line:\n\"" + str_buf0 + "\"");
    ...
    LOGGER << "Phenotypic variance estimated from summary statistics of all " << count << " SNPs: " << _jma_Vp
           << " (variance of logit for case-control studies)." << endl;
    if (GC) {
        if (GC_val > 0) {
            _GC_val = GC_val;
    ...
    for (i = 0; i < snplist.size(); i++)
        id_map.insert(pair<string, int>(snplist[i], i));
    for (i = 0; i < _include.size(); i++) {
        int include_i = _include[i];
        bool flip_flag = false;
        string cur_snp_name = _snp_name[include_i];
        auto id_it = id_map.find(cur_snp_name);
        _ref_A[include_i] = ref_A1_buf[id_it->second];
        _other_A[include_i] = ref_A2_buf[id_it->second];
        if (!_mu.empty() && ref_A1_buf[id_it->second] == _allele2[include_i]) {
            _mu[include_i] = 2.0 - _mu[include_i];
            flip_flag = true;
        }
        double cur_freq_value = _mu[include_i] / 2.0;
        double freq_diff = abs(cur_freq_value - freq_buf[id_it->second]);
        if (freq_diff < freq_diff_thresh) {
            snplist_freq.push_back(cur_snp_name);
            indx.push_back(id_it->second);
        } else {
            if (flip_flag)
                cur_freq_value = 1.0 - cur_freq_value;
            ...
            bad_freq_ma.push_back(freq_buf[id_it->second]);
        }
    }
    ...
    if (!bad_snp.empty()) {
        ...
        for (i = 0; i < bad_snp.size(); i++)
            obadsnp << bad_snp[i] << "\t" << bad_A1[i] << "\t" << bad_A2[i] << "\t" << bad_refA[i] << endl;
        obadsnp.close();
        LOGGER << "Warning: can't match the reference alleles of " << bad_snp.size()
               << " SNPs to those in the genotype data. These SNPs have been saved in [" + badsnpfile + "]." << endl;
    }
    if (!bad_snp_freq.empty()) {
        ...
        for (int i = 0; i < bad_snp_freq.size(); i++) {
            obadsnp << bad_snp_freq[i] << "\t" << bad_refA_freq[i] << "\t" << bad_otherA_freq[i] << "\t"
                    << bad_freq_ma[i] << "\t" << bad_A1_freq[i] << "\t" << bad_A2_freq[i] << "\t" << bad_freq_value[i]
                    << endl;
        }
        obadsnp.close();
        LOGGER << bad_snp_freq.size()
               << " SNP(s) have large difference of allele frequency between the GWAS summary data and the reference "
                  "sample. These SNPs have been saved in ["
               << badsnpfile << "]." << endl;
    }
    ...
    if (_include.empty())
        ...
        LOGGER << _include.size() << " SNPs are matched to the genotype data." << endl;
    ...
    for (i = 0; i < _include.size(); i++) {
        ...
        if (GC) {
            ...
            _pval[i] = StatFunc::pchisq(_chisq[i], 1);
...
#pragma omp parallel for
    ...
    if (_jma_actual_geno) {
        ...
        for (i = 0; i < m; i++)
            _Nd[i] = (_jma_Vp - _MSX[i] * _beta[i] * _beta[i]) / (_MSX[i] * _beta_se[i] * _beta_se[i]) +
                     1; // revised by JY 25/11/13 according to Eq. 13, Yang et al. 2012 NG
    }
    build_cojo_bp_order();
}
...
#pragma omp parallel for
    ...
    LOGGER << "COJO bit-packed genotypes enabled (" << m << " SNPs × " << n << " individuals, " << _cojo_words
           << " words/SNP)." << endl;
}
...
double gcta::cojo_snp_dot_bits(int i, int j) const {
    ...
    for (size_t w = 0; w < _cojo_words; w++) {
        ...
        S12 += __builtin_popcountll(x11 & x21) + __builtin_popcountll(x11 & x22) + __builtin_popcountll(x12 & x21) +
               __builtin_popcountll(x12 & x22);
...
void gcta::cojo_window_bounds(int snp_include_idx, int& lo, int& hi) const {
    ...
    lo = 0;
    hi = m;
    if (m == 0 || snp_include_idx < 0 || snp_include_idx >= (int)_include.size())
        return;
    const int chr = _chr[_include[snp_include_idx]];
    const int bp = _bp[_include[snp_include_idx]];

    int c_lo = 0, c_hi = m;
    while (c_lo < m && _chr[_include[_bp_order[c_lo]]] < chr)
        c_lo++;
    c_hi = c_lo;
    while (c_hi < m && _chr[_include[_bp_order[c_hi]]] == chr)
        c_hi++;
    if (c_lo == c_hi) {
        lo = hi = c_lo;
        return;
...
void gcta::read_fixed_snp(string snplistfile, string msg, vector<int>& pgiven, vector<int>& remain) {
    ...
    if (givenSNPs.empty())
        LOGGER.e(0, "failed to read any SNP from the file [" + snplistfile + "].");
    map<string, int> givenSNPs_map;
    pgiven.clear();
    remain.clear();
    for (i = 0; i < givenSNPs.size(); i++)
        givenSNPs_map.insert(pair<string, int>(givenSNPs[i], i));
    for (i = 0; i < _include.size(); i++) {
        if (givenSNPs_map.find(_snp_name[_include[i]]) != givenSNPs_map.end())
            pgiven.push_back(i);
        else
            remain.push_back(i);
    }
    if (pgiven.size() > 0)
        LOGGER << pgiven.size() << " of them are matched to the genotype and summary data." << endl;
    else
        LOGGER.e(0, "none of the given SNPs can be matched to the genotype and summary data.");
}

void gcta::reset_cojo_profile() {
    _cojo_prof_massoc_cond_ns = 0;
    _cojo_prof_insert_ok_ns = 0;
    _cojo_prof_insert_reject_ns = 0;
    _cojo_prof_slct_stay_ns = 0;
    _cojo_prof_n_massoc_cond = 0;
    _cojo_prof_n_insert_ok = 0;
    _cojo_prof_n_insert_reject = 0;
}

void gcta::report_cojo_profile() {
    const double cond_s = _cojo_prof_massoc_cond_ns / 1e9;
    const double ins_ok_s = _cojo_prof_insert_ok_ns / 1e9;
    const double ins_rej_s = _cojo_prof_insert_reject_ns / 1e9;
    const double stay_s = _cojo_prof_slct_stay_ns / 1e9;
    const double total_s = cond_s + ins_ok_s + ins_rej_s + stay_s;
    LOGGER << "COJO profile: massoc_cond=" << cond_s << "s (n=" << _cojo_prof_n_massoc_cond
           << ") insert_ok=" << ins_ok_s << "s (n=" << _cojo_prof_n_insert_ok << ") insert_reject=" << ins_rej_s
           << "s (n=" << _cojo_prof_n_insert_reject << ") slct_stay=" << stay_s << "s; profiled_total=" << total_s
           << "s"
           << "; collinear_filtered=" << _jma_snpnum_collienar << endl;
    if (_out.empty())
        return;
    const string path = _out + ".cojo_timing.tsv";
    ofstream ofs(path.c_str());
    if (!ofs)
        return;
    ofs << "phase\tseconds\tcount\n";
    ofs << "massoc_cond\t" << cond_s << "\t" << _cojo_prof_n_massoc_cond << "\n";
    ofs << "insert_ok\t" << ins_ok_s << "\t" << _cojo_prof_n_insert_ok << "\n";
    ofs << "insert_reject\t" << ins_rej_s << "\t" << _cojo_prof_n_insert_reject << "\n";
    ofs << "slct_stay\t" << stay_s << "\t" << 0 << "\n";
    ofs << "profiled_total\t" << total_s << "\t" << 0 << "\n";
    ofs << "collinear_filtered\t0\t" << _jma_snpnum_collienar << "\n";
    LOGGER << "COJO profile written to [" << path << "]." << endl;
}

void gcta::run_massoc_slct(string metafile,
                           int wind_size,
                           double p_cutoff,
                           double collinear,
                           int64_t top_SNPs,
                           ...
                           bool actual_geno,
                           int mld_slct_alg) {
    // OpenMP parallelizes COJO outer loops (Z fill, massoc_cond). Keep Eigen/BLAS
    // single-threaded so nested reductions cannot change LD / pC floating-point
    // association order (otherwise --thread-num>1 is nondeterministic on SNP sets).
    ...
    if (!joint_only && mld_slct_alg < 2) {
        LOGGER << "Performing " << ((mld_slct_alg == 0) ? "stepwise" : "forward") << " model selection on "
               << _include.size() << " SNPs to select association signals ... (p cutoff = " << _jma_p_cutoff << "; ";
        LOGGER << "collinearity cutoff = " << _jma_collinear << ")" << endl;
        if (!_jma_actual_geno)
            LOGGER << "(Assuming complete linkage equilibrium between SNPs which are more than " << _jma_wind_size / 1e6
                   << "Mb away from each other)" << endl;
        ...
        for (i = 0; i < _include.size(); i++)
            slct.push_back(i);
        if (mld_slct_alg == 2) {
            LOGGER << "Performing backward selection on " << _include.size()
                   << " SNPs at threshold p-value = " << _jma_p_cutoff << " ..." << endl;
    ...
    if (joint_only)
        ...
        LOGGER << " selected signals ..." << endl;
    if (slct.size() >= _keep.size())
        LOGGER.e(0, "too many SNPs. The number of SNPs in a joint analysis should not be larger than the sample size.");
    massoc_joint(slct, bJ, bJ_se, pJ);
    eigenMatrix rval(slct.size(), slct.size());
    LD_rval(slct, rval);
    if (_jma_actual_geno)
        LOGGER << "Residual variance = " << _jma_Ve << endl;
    massoc_slct_output(joint_only, slct, bJ, bJ_se, pJ, rval);

    // output conditional results
    if (!joint_only && mld_slct_alg != 2) {
        massoc_cond_output(remain, bC, bC_se, pC);
        LOGGER << "(" << _jma_snpnum_backward << " SNPs eliminated by backward selection and " << _jma_snpnum_collienar
               << " SNPs filtered by collinearity test are not included in the output)" << endl;
    }
    report_cojo_profile();
}

void gcta::run_massoc_cond(
    string metafile, string snplistfile, int wind_size, double collinear, bool GC, double GC_val, bool acutal_geno) {
    ...
    for (i = 0; i < pgiven.size(); i++) {
        ...
        ofile << _chr[_include[j]] << "\t" << _snp_name[_include[j]] << "\t" << _bp[_include[j]] << "\t"
              << _ref_A[_include[j]] << "\t" << _freq[j] << "\t" << _beta[j] << "\t" << _beta_se[j] << "\t" << _pval[j]
              << endl;
    }
    ofile.close();
...
void gcta::massoc_slct_output(
    bool joint_only, vector<int>& slct, eigenVector& bJ, eigenVector& bJ_se, eigenVector& pJ, eigenMatrix& rval) {
    string filename = _out + ".jma.cojo";
    if (joint_only)
        LOGGER << "Saving the joint analysis result of " << slct.size() << " SNPs to [" + filename + "] ..." << endl;
    else
        LOGGER << "Saving the " << slct.size() << " independent signals to [" + filename + "] ..." << endl;
    ofstream ofile(filename.c_str());
    if (!ofile)
        LOGGER.e(0, "cannot open the file [" + filename + "] to write.");
    ofile << "Chr\tSNP\tbp\trefA\tfreq\tb\tse\tp" << ((_GC_val > 0) ? "_GC" : "") << "\tn\tfreq_geno\tbJ\tbJ_se\tpJ"
          << ((_GC_val > 0) ? "_GC" : "") << "\tLD_r" << endl;
    int i = 0, j = 0;
    for (i = 0; i < slct.size(); i++) {
        j = slct[i];
        ofile << _chr[_include[j]] << "\t" << _snp_name[_include[j]] << "\t" << _bp[_include[j]] << "\t";
        ofile << _ref_A[_include[j]] << "\t" << _freq[j] << "\t" << _beta[j] << "\t" << _beta_se[j] << "\t";
        ofile << _pval[j] << "\t" << _Nd[j] << "\t" << 0.5 * _mu[_include[j]] << "\t" << bJ[i] << "\t" << bJ_se[i]
              << "\t" << pJ[i] << "\t";
        if (i == slct.size() - 1)
            ofile << 0 << endl;
        else
    ...
    if (joint_only)
        LOGGER << "Saving the LD structure of " << slct.size() << " SNPs to [" + filename + "] ..." << endl;
    else
        LOGGER << "Saving the LD structure of " << slct.size() << " independent signals to [" + filename + "] ..."
               << endl;
...
void gcta::massoc_cond_output(vector<int>& remain, eigenVector& bC, eigenVector& bC_se, eigenVector& pC) {
    ...
    LOGGER << "Saving the conditional analysis results of " << remain.size()
           << " remaining SNPs to [" + filename + "] ..." << endl;
    ofstream ofile(filename.c_str());
    if (!ofile)
        LOGGER.e(0, "cannot open the file [" + filename + "] to write.");
    ofile << "Chr\tSNP\tbp\trefA\tfreq\tb\tse\tp" << ((_GC_val > 0) ? "_GC" : "") << "\tn\tfreq_geno\tbC\tbC_se\tpC"
          << ((_GC_val > 0) ? "_GC" : "") << endl;
...
void gcta::stepwise_slct(vector<int>& slct,
                         vector<int>& remain,
                         eigenVector& bC,
                         eigenVector& bC_se,
                         eigenVector& pC,
    ...
    while (!remain.empty()) {
        ...
        if (!slct_only_contain_remain) {
            if (slct.size() % 5 == 0 && slct.size() > prev_num)
                LOGGER << slct.size() << " associated SNPs have been selected." << endl;
            if (slct.size() > prev_num)
                prev_num = slct.size();
            if (slct.size() >= top_SNPs)
                break;
        }
    }
    if (slct_only_contain_remain) {
...
bool gcta::slct_entry(vector<int>& slct, vector<int>& remain, eigenVector& bC, eigenVector& bC_se, eigenVector& pC) {
    ...
    int i = 0, m = 0;
    {
        const auto t0 = CojoClock::now();
        massoc_cond(slct, remain, bC, bC_se, pC);
        _cojo_prof_massoc_cond_ns += cojo_ns_since(t0);
        _cojo_prof_n_massoc_cond++;
    }
    vector<double> pC_buf;
    eigenVector2Vector(pC, pC_buf);
    while (true) {
        m = min_element(pC_buf.begin(), pC_buf.end()) - pC_buf.begin();
        if (pC_buf[m] >= _jma_p_cutoff)
            return (false);
        bool inserted = false;
        {
            const auto t0 = CojoClock::now();
            inserted = insert_B_and_Z(slct, remain[m]);
            const long long dt = cojo_ns_since(t0);
            if (inserted) {
                _cojo_prof_insert_ok_ns += dt;
                _cojo_prof_n_insert_ok++;
            } else {
                _cojo_prof_insert_reject_ns += dt;
                _cojo_prof_n_insert_reject++;
            }
        }
        if (inserted) {
...
void gcta::slct_stay(vector<int>& slct, eigenVector& bJ, eigenVector& bJ_se, eigenVector& pJ) {
    ...
    if (_B_N.cols() < 1) {
        if (!init_B(slct))
            LOGGER.e(0,
                     "there is a collinearity problem of the given list of SNPs.\nYou can try the option --cojo-slct "
                     "to remove one of each pair of highly correlated SNPs.");
    }
    ...
    _cojo_prof_slct_stay_ns += cojo_ns_since(t0);
}
...
double gcta::massoc_calcu_Ve(const vector<int>& slct, eigenVector& bJ, eigenVector& b) {
    ...
    if (d_buf - n < 1)
        LOGGER.e(0,
                 "no degree of freedom is left for the residues. The model is over-fitted. Please specify a more "
                 "stringent p-value cut-off.");
    Ve = ((d_buf - 1) * _jma_Vp - Ve) / (d_buf - n);
    if (Ve <= 0.0)
        LOGGER.e(0,
                 "residual variance is out of boundary. The model is over-fitted. Please specify a more stringent "
                 "p-value cut-off.");
    return Ve;
}
...
void gcta::massoc_joint(const vector<int>& indx, eigenVector& bJ, eigenVector& bJ_se, eigenVector& pJ) {
    ...
    if (_jma_actual_geno)
        _jma_Ve = massoc_calcu_Ve(indx, bJ, b);
    bJ_se *= _jma_Ve;
    for (i = 0; i < n; i++) {
        if (bJ_se[i] > 1.0e-30) {
            ...
            chisq = bJ[i] / bJ_se[i];
            if (_GC_val > 0)
                pJ[i] = StatFunc::pchisq(chisq * chisq / _GC_val, 1);
            else
...
void gcta::massoc_cond(
    const vector<int>& slct, const vector<int>& remain, eigenVector& bC, eigenVector& bC_se, eigenVector& pC) {
    if (slct.size() < 1) {
        LOGGER.e(0, "Warning, this should not happend");
    }
    if (_B_N.cols() < 1) {
    ...
    if (_jma_actual_geno)
        _jma_Ve = massoc_calcu_Ve(slct, bJ1, b);

    bC = eigenVector::Zero(remain.size());
    bC_se = eigenVector::Zero(remain.size());
    pC = eigenVector::Constant(remain.size(), 2);
    const double cutoff = 1e-10 * _jma_Vp;
    const int n_remain = static_cast<int>(remain.size());
    const bool actual_geno = _jma_actual_geno;
    const double jma_Ve = _jma_Ve;
    const double gc_val = _GC_val;
    const double jma_collinear = _jma_collinear;
    const int n_slct = n;
    // dcdflib (StatFunc::pchisq) uses file-scope mutable statics — not thread-safe.
    // Parallelize the heavy Z / B_i work; compute p-values serially afterward.
    eigenVector chi2_buf = eigenVector::Constant(n_remain, -1.0);
    const int k = _Z_nrows > 0 ? _Z_nrows : static_cast<int>(_B_i.rows());
    // Precompute once per step: zn.dot(v_bc) == (zn^T * B_N_i) ∘ D_N · b (B_N_i SPD ⇒ symmetric).
    const eigenVector v_bc = _B_N_i * _D_N.cwiseProduct(b);

#pragma omp parallel for schedule(static)
    for (int ii = 0; ii < n_remain; ii++) {
        const int j = remain[ii];
        const double B2 = _MSX[j] * _Nd[j];
        double bC_i = 0.0;
        double bC_se_i = 0.0;
        if (!CommFunc::FloatEqual(B2, 0.0)) {
            const bool maybe_nz = _Z_col_has_nz.empty() || _Z_col_has_nz[j];
            if (!maybe_nz) {
                bC_i = _beta[j];
                bC_se_i = 1.0 / B2;
            } else {
                const eigenVector z = _Z_cache.col(j).head(k);
                const eigenVector zn = _Z_N_cache.col(j).head(k);
                const eigenVector Z_Bi_buf = z.transpose() * _B_i;
                if (z.dot(Z_Bi_buf) / _MSX_B[j] < jma_collinear) {
                    bC_i = _beta[j] - zn.dot(v_bc) / B2;
                    bC_se_i = 1.0 / B2;
                }
            }
        }
        if (actual_geno)
            bC_se_i *= jma_Ve - (B2 * bC_i * _beta[j]) / (_Nd[j] - n_slct - 1);
        else
            bC_se_i *= jma_Ve;
        if (bC_se_i > cutoff) {
            bC_se_i = sqrt(bC_se_i);
            const double chisq = bC_i / bC_se_i;
            chi2_buf[ii] = (gc_val > 0) ? (chisq * chisq / gc_val) : (chisq * chisq);
        }
        bC[ii] = bC_i;
        bC_se[ii] = bC_se_i;
    }
    for (int ii = 0; ii < n_remain; ii++) {
        if (chi2_buf[ii] >= 0.0)
            pC[ii] = StatFunc::pchisq(chi2_buf[ii], 1);
        else
            pC[ii] = 2.0;
    }
}
...
bool gcta::init_B(const vector<int>& indx) {
    if (indx.size() < 1) {
        LOGGER.e(0,
                 "slct size is zero will cause Eigen Matrix of"
    ...
    for (i = 0; i < indx.size(); i++) {
        ...
        for (j = i + 1; j < indx.size(); j++) {
            if (_jma_actual_geno || (_chr[_include[indx[i]]] == _chr[_include[indx[j]]] &&
                                     abs(_bp[_include[indx[i]]] - _bp[_include[indx[j]]]) < _jma_wind_size)) {
                d_buf = cojo_snp_dot(indx[i], indx[j]);
                _B.insertBack(j, i) = d_buf;
                _B_N.insertBack(j, i) = d_buf * min(_Nd[indx[i]], _Nd[indx[j]]) *
                                        sqrt(_MSX[indx[i]] * _MSX[indx[j]] / (_MSX_B[indx[i]] * _MSX_B[indx[j]]));
            }
        }
    }
    _B.finalize();
    ...
    if ((1 - eigenVector::Constant(indx.size(), 1).array() / (diagB.array() * _B_i.diagonal().array())).maxCoeff() >
        _jma_collinear)
        return false;
...
void gcta::z_cache_ensure_physical_rows(int need_rows, int m) {
    if (need_rows <= 0)
        return;
    if (_Z_cache.cols() == m && _Z_cache.rows() >= need_rows)
        return;
    const int old_rows = static_cast<int>(_Z_cache.rows());
    int new_rows = need_rows;
    if (old_rows > 0 && _Z_cache.cols() == m) {
        new_rows = old_rows;
        while (new_rows < need_rows)
            new_rows *= 2;
    }
    eigenMatrix Z_new = eigenMatrix::Zero(new_rows, m);
    eigenMatrix ZN_new = eigenMatrix::Zero(new_rows, m);
    const int copy_rows = std::min(old_rows, _Z_nrows > 0 ? _Z_nrows : old_rows);
    if (copy_rows > 0 && _Z_cache.cols() == m) {
        Z_new.topRows(copy_rows) = _Z_cache.topRows(copy_rows);
        ZN_new.topRows(copy_rows) = _Z_N_cache.topRows(copy_rows);
    }
    _Z_cache.swap(Z_new);
    _Z_N_cache.swap(ZN_new);
}

void gcta::z_cache_rebuild_col_has_nz() {
    const int m = static_cast<int>(_include.size());
    _Z_col_has_nz.assign(m, 0);
    if (!_Z_cache_ready || _Z_nrows <= 0)
        return;
    for (int r = 0; r < _Z_nrows; r++) {
        for (int j = 0; j < m; j++) {
            if (_Z_cache(r, j) != 0.0)
                _Z_col_has_nz[j] = 1;
        }
...
void gcta::fill_z_row_for_selected(int row, int selected_include_idx) {
    const int m = static_cast<int>(_include.size());
    if (row < 0 || row >= _Z_cache.rows() || selected_include_idx < 0 || selected_include_idx >= m)
        return;
    if ((int)_Z_col_has_nz.size() != m)
        _Z_col_has_nz.assign(m, 0);
...
#pragma omp parallel for schedule(static)
    for (int t = lo; t < hi; t++) {
        ...
        if (!_jma_actual_geno) {
            if (selected_include_idx == j)
                continue;
            if (_chr[_include[selected_include_idx]] != _chr[_include[j]])
                continue;
            if (abs(_bp[_include[selected_include_idx]] - _bp[_include[j]]) >= _jma_wind_size)
                continue;
        }
...
void gcta::append_z_cache_row(int insert_include_idx, int sorted_pos) {
    const int m = static_cast<int>(_include.size());
    const int k = _Z_nrows;
    z_cache_ensure_physical_rows(k + 1, m);
...
void gcta::drop_z_cache_row(int erase_pos) {
    ...
    const int k = _Z_nrows;
    if (erase_pos < 0 || erase_pos >= k)
        return;
    for (int r = erase_pos; r < k - 1; r++) {
        _Z_cache.row(r) = _Z_cache.row(r + 1);
        _Z_N_cache.row(r) = _Z_N_cache.row(r + 1);
    }
    _Z_cache.row(k - 1).setZero();
    _Z_N_cache.row(k - 1).setZero();
    _Z_nrows = k - 1;
    z_cache_rebuild_col_has_nz();
}
...
void gcta::init_Z(const vector<int>& indx) {
    ...
    const int m = static_cast<int>(_include.size());
    z_cache_reset();
    const int cap = std::max(k, std::min(k * 2, k + 64));
    z_cache_ensure_physical_rows(cap, m);
    _Z_col_has_nz.assign(m, 0);
    _Z_nrows = k;
    for (int i = 0; i < k; i++) {
...
bool gcta::insert_B_and_Z(const vector<int>& indx, int insert_indx) {
    ...
    for (j = 0; j < ix.size(); j++) {
        ...
        for (i = j + 1; i < ix.size(); i++) {
            ...
            if (insert_indx == ix[j] || insert_indx == ix[i]) {
                if (_jma_actual_geno || (_chr[_include[ix[i]]] == _chr[_include[ix[j]]] &&
                                         abs(_bp[_include[ix[i]]] - _bp[_include[ix[j]]]) < _jma_wind_size)) {
                    d_buf = cojo_snp_dot(ix[i], ix[j]);
                    _B.insertBack(i, j) = d_buf;
                    _B_N.insertBack(i, j) = d_buf * min(_Nd[ix[i]], _Nd[ix[j]]) *
                                            sqrt(_MSX[ix[i]] * _MSX[ix[j]] / (_MSX_B[ix[i]] * _MSX_B[ix[j]]));
                }
            } else {
    ...
    const bool can_incr = (_B_i.rows() == (int)indx.size() && _B_i.cols() == (int)indx.size() &&
                           _B_N_i.rows() == (int)indx.size() && _B_N_i.cols() == (int)indx.size() && indx.size() > 0);
    ...
    if (can_incr) {
        ...
        // Collinearity first (O(k)) — rejects most candidates on dense chromosomes —
        // then sparse SimplicialLDLT for PD/cond (stock D-ratio; keep until a
        // proven-parity LDLT-free estimate exists).
        const double max_col =
            (1 - eigenVector::Constant(ix.size(), 1).array() / (diagB.array() * _B_i.diagonal().array())).maxCoeff();
        if (max_col > _jma_collinear) {
            _jma_snpnum_collienar++;
            _B = B_buf;
            _B_N = B_N_buf;
            _B_i = B_i_buf;
...
void gcta::LD_rval(const vector<int>& indx, eigenMatrix& rval) {
    ...
    for (i = 0; i < indx.size(); i++)
        sd[i] = sqrt(_MSX_B[indx[i]]);
    for (j = 0; j < indx.size(); j++) {
        rval(j, j) = 1.0;
        for (i = j + 1; i < indx.size(); i++)
            rval(i, j) = rval(j, i) = _B.coeff(i, j) / sd[i] / sd[j];
    }
}
...
void gcta::run_massoc_sblup(string metafile, int wind_size, double lambda) {
    ...
    if (massoc_sblup(lambda, bJ)) {
        ...
        LOGGER << "Saving the joint analysis result of " << _include.size() << " SNPs to [" + filename + "] ..."
               << endl;
        ofstream ofile(filename.c_str());
        if (!ofile)
            LOGGER.e(0, "cannot open the file [" + filename + "] to write.");
        for (j = 0; j < _include.size(); j++) {
            ...
            ofile << _snp_name[_include[j]] << "\t" << _ref_A[_include[j]] << "\t" << _beta[j] << "\t"
                  << bJ[j] / sqrt(_MSX[j]) << endl;
        }
        ofile.close();
    } else
        LOGGER.e(0,
                 "Jacobi iteration cannot converge. You can increase the maximum number of iterations by the option "
                 "--massoc-sblup-maxit.");
}
...
#pragma omp ordered
                ...
                // out_debug << j << "\t" << i << "\t" << _snp_name[_include[j]] << "\t" << _snp_name[_include[i]] <<
                // "\t" << temp << endl;
            }
        }
        if ((i + 1) % 1000 == 0 || (i + 1) == _include.size())
            LOGGER << i + 1 << " of " << _include.size() << " SNPs.\r";
    }
    // out_debug.close();
